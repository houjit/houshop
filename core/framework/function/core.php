...
function getFlexigridArray($in_array, $fields_array, $data, $format_array)//xxxxx.
{
	$out_array = $in_array;
...
function ncPriceFormatb($price)//xxxxx.
{
 ...
 * @return void
 */
function requestNotFound($app = null, $ops = null) {
 ...
 * @return string 字符串类型的返回结果
 */
function getNchash($app = '', $ops = ''){
    $app = $app ? $app : $_GET['app'];
    $ops = $ops ? $ops : ($_GET['ops'] ? $_GET['ops'] : $_GET['feiwa']);
    if (C('captcha_status_login')){
        return substr(md5(MALL_SITE_URL.$app.$ops),0,8);
    } else {
        return '';
    }
}
...
function write_file($filepath, $data, $mode = null)
{
    ...
    $data = "<?php defined('INSYSVIEW') or exit('Access Invalid!'); return ".$data.";";
 ...
 * @return string
 */
function url($app = '', $ops = '', $args = array(), $model = false, $site_url = ''){
    //伪静态文件扩展名
    $ext = '.html';
    //入口文件名
    $file = 'index.php';
//    $site_url = empty($site_url) ? MALL_SITE_URL : $site_url;
    ...
    //定义变量存放返回url
    $url_string = '';
    if (empty($app) && empty($ops) && empty($args)) {
        return $site_url;
    }
    $app = !empty($app) ? $app : 'index';
    $ops = !empty($ops) ? $ops : 'index';
 ...
 *
 * @param string $app control文件名
 * @param string $ops op方法名
 * @param array $args URL其它参数
 * @param string $store_domian 店铺二级域名
 * @return string
 */
function urlMall($app = '', $ops = '', $args = array(), $store_domain = ''){

    // 如何使自营店则返回javascript:;

//    if ($app == 'show_store' && $ops != 'goods_all') {
...
//    }
    ...
    if(URL_MODEL && in_array($app.':'.$ops, $rewrite_item)) {
        $rewrite_flag = true;
    ...
    return url($app, $ops, $args, $rewrite_flag, MALL_SITE_URL);
}
 ...
 * @return string
 */
function urlAdmin($app = '', $ops = '', $args = array()){
    return url($app, $ops, $args, false, ADMIN_SITE_URL);
}
 ...
 * @return string
 */
function urlAdminMall($app = '', $ops = '', $args = array()){
    return url($app, $ops, $args, false, ADMIN_modules_URL);
}
/**
 ...
 * @return string
 */
function urlAdminCircle($app = '', $ops = '', $args = array()){
    return url($app, $ops, $args, false, CIRCLE_modules_URL);
}
/**
 ...
 */
function urlAdminMobile($app = '', $ops = '', $args = array()){
    return url($app, $ops, $args, false, MOBILE_modules_URL);
}
/**
 ...
 * @return stringurlAdminMobile
 */
function urlAdminReads($app = '', $ops = '', $args = array()){
    return url($app, $ops, $args, false, READS_modules_URL);
}
/**
 ...
 * @return stringurlAdminMobile
 */
function urlAdminShareShow($app = '', $ops = '', $args = array()){
    return url($app, $ops, $args, false, SHARESHOW_modules_URL);
}
/**
 ...
 * @return string
 */
function urlREADS($app = '', $ops = '', $args = array()){
	// 如果平台开启伪静态开关，并且为伪静态模块，修改标志为开启伪静态
    $rewrite_item = array(
    ...
    return url($app, $ops, $args, $rewrite_flag, READS_SITE_URL);
}
/**
 ...
 * @return string
 */
function urlCircle($app = '', $ops = '', $args = array()){
	// 如果平台开启伪静态开关，并且为伪静态模块，修改标志为开启伪静态
    $rewrite_item = array(
	        'index:index',
	        'theme:theme_detail'
    ...
    return url($app, $ops, $args, $rewrite_flag, CIRCLE_SITE_URL);
}
/**
 ...
 * @return string
 */
function urlShareShow($app = '', $ops = '', $args = array()){
    return url($app, $ops, $args, false, SHARESHOW_SITE_URL);
}
/**
 ...
 * @return string
 */
function urlMember($app = '', $ops = '', $args = array()) {
    // 默认标志为不开启伪静态
    $rewrite_flag = false;
    ...
    return url($app, $ops, $args, $rewrite_flag, MEMBER_SITE_URL);
}
/**
 ...
 * @return string
 */
function urlLogin($app = '', $ops = '', $args = array()) {
	    // 如果平台开启伪静态开关，并且为伪静态模块，修改标志为开启伪静态
    $rewrite_item = array(
	        'login:index',
			'login:register'
    ...
    return url($app, $ops, $args, false, LOGIN_SITE_URL);
}
/**
 ...
 * @return string
 */
function urlChain($app = '', $ops = '', $args = array()){
    return url($app, $ops, $args, false, CHAIN_SITE_URL);
}
/**
 * 验证是否为平台店铺
 *
