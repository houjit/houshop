...
//	}
	...
	$compile .= "\nBase::run();";
	file_put_contents(RUNCOREPATH,compress_code("<?php defined('INSYSVIEW') or exit('Access Invalid!');".$compile));
}
...
function compile($filename) {
    ...
    //过滤不需要编译的内容
    $content = preg_replace('/\/\/\[FEIWA_SKIPBUILD\](.*?)\/\/\[\/FEIWA_SKIPBUILD\]/s', '', $content);
    $content = str_ireplace("defined('INSYSVIEW') or exit('Access Invalid!')", '', $content);
