 ...
 *
 */
defined('INSYSVIEW') or exit('Access Invalid!cccccccccc');
...
class Route {
    ...
    private function _parseRuleRewrite() {
        ...
        if (!empty($path_info) && $path_info != 'index.php' && strpos($path_info, $this->_rewrite_extname)){
            ...
            //app,op强制赋值
            $_GET['app'] = isset($split_array[0]) ? $split_array[0] : 'index';
            //$_GET['feiwa'] = isset($split_array[1]) ? $split_array[1] : 'index';
            $_GET['ops'] = isset($split_array[1]) ? $split_array[1] : 'index';
            unset($split_array[0]);
            unset($split_array[1]);
     ...
     * @return mixed
     */
    private function _mallPathInfo($path_info) {
        $reg_match_from = array(
            ...
            '/^c-(\d+)$/',
            '/^t-(\d+)$/',
            '/^mall_view-(\d+)-(\d+)-([0-5])-([0-2])-(\d+)$/',
            '/^article-(\d+)$/',
            '/^article_cate-(\d+)$/',
            '/^document-([a-z_]+)$/',
            '/^s-(\d+)-([0-9_]+)-([0-9_]+)-([0-3])-([0-2])-([0-1])-([0-1])-(\d+)-(\d+)$/',
			'/^channel-(\d+)$/',
     ...
     * @return mixed
     */
    private function _readsPathInfo($path_info) {
        $reg_match_from = array(
            '/^article-(\d+)$/',
            '/^article_list-(\d+)$/',
            '/^picture-(\d+)$/'
        ...
        $reg_match_to = array(
            ...
            'article-article_list-class_id-\\1',
     ...
     * @return mixed
     */
    private function _circlePathInfo($path_info) {
        $reg_match_from = array(
