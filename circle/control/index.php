...
class indexControl extends BaseCircleControl{
    ...
    public function indexFeiwa(){
        ...
        // 推荐圈子      
        $circle_list = $model->table('circle')->where(array('circle_status'=>1, 'is_recommend'=>1))->order('circle_id desc')->limit(10)->select();
        ...
        //首页话题列表
        $circles_list = $model->table('circle_theme')->where($where)->order($order)->limit(20)->select();
		$circles_list = array_under_reset($circles_list, 'theme_id');
        Tpl::output('circles_list', $circles_list);
        // 附件列表
        if(!empty($circles_list)){
            $themeid_array = array_keys($circles_list);
            $affix_lists = $model->table('circle_affix')->where(array('affix_type'=>1,'theme_id'=>array('in', $themeid_array)))->select();
        ...
        // 推荐话题
        $theme_list = $model->table('circle_theme')->where(array('has_affix'=>1, 'is_closed'=>0, 'is_recommend'=>1))->limit(8)->select();
        if(!empty($theme_list)){
        ...
        // 优秀成员
        $member_list = $model->table('circle_member')->where(array('is_recommend'=>1))->limit(5)->select();
     ...
     * 创建圈子
     */
    public function add_groupFeiwa(){
        ...
        if(chksubmit()){
            ...
            if($error != ''){
                ...
                if($result){
                    ...
                    showDialog(L('feiwa_common_op_succ'),'index.php?app=group&c_id='.$result, 'succ');
                }else{
     ...
     * 我加入的圈子
     */
    public function myjoinedcircleFeiwa(){
