...
Tpl::showpage('mb_special.list');
     ...
     * 保存专题
     */
    public function special_saveFeiwa() {
        $model_mb_special = Model('mb_special');
        ...
        if($result) {
            ...
            showMessage(L('feiwa_common_save_succ'), urlAdminMobile('mb_special', 'special_list'));
        } else {
            $this->log('添加手机专题' . '[ID:' . $result. ']', 0);
            showMessage(L('feiwa_common_save_fail'), urlAdminMobile('mb_special', 'special_list'));
        }
    }
     ...
     * 编辑专题描述
     */
    public function update_special_descFeiwa() {
        $model_mb_special = Model('mb_special');
     ...
     * 删除专题
     */
    public function special_delFeiwa() {
        $model_mb_special = Model('mb_special');
        ...
        if($result) {
            ...
            showMessage(L('feiwa_common_del_succ'), urlAdminMobile('mb_special', 'special_list'));
        } else {
            $this->log('删除手机专题' . '[ID:' . $_POST['special_id'] . ']', 0);
            showMessage(L('feiwa_common_del_fail'), urlAdminMobile('mb_special', 'special_list'));
        }
    }
     ...
     * 编辑首页
     */
    public function index_editFeiwa() {
        $model_mb_special = Model('mb_special');
...
Tpl::showpage('mb_special_item.list');
     ...
     * 编辑专题
     */
    public function special_editFeiwa() {
        $model_mb_special = Model('mb_special');
...
Tpl::showpage('mb_special_item.list');
     ...
     * 专题项目添加
     */
    public function special_item_addFeiwa() {
        $model_mb_special = Model('mb_special');
     ...
     * 专题项目删除
     */
    public function special_item_delFeiwa() {
        $model_mb_special = Model('mb_special');
     ...
     * 专题项目编辑
     */
    public function special_item_editFeiwa() {
        $model_mb_special = Model('mb_special');
     ...
     * 专题项目保存
     */
    public function special_item_saveFeiwa() {
        $model_mb_special = Model('mb_special');
        ...
        if($result) {
            if($_POST['special_id'] == $model_mb_special::INDEX_SPECIAL_ID) {
                showMessage(L('feiwa_common_save_succ'), urlAdminMobile('mb_special', 'index_edit'));
            } else {
                showMessage(L('feiwa_common_save_succ'), urlAdminMobile('mb_special', 'special_edit', array('special_id' => $_POST['special_id'])));
            }
        } else {
     ...
     * 限时折扣商品列表
     */
    public function goods_xianshi_listFeiwa() {
        $model_goods = Model('goods');
        $condition = array();
     ...
     * 团购商品列表
     */
    public function goods_groupbuy_listFeiwa() {
        $model_goods = Model('goods');
        $condition = array();
        $condition['goods_name'] = array('like', '%' . $_GET['keyword'] . '%');
     ...
     * 更新项目排序
     */
    public function update_item_sortFeiwa() {
     ...
     * 更新项目启用状态
     */
    public function update_item_usableFeiwa() {
        $model_mb_special = Model('mb_special');
