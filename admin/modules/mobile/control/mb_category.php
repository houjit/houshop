...
class mb_categoryControl extends SystemControl{
    ...
    public function indexFeiwa() {
        $this->mb_category_listFeiwa();
    }
    /**
     *
     */
    public function mb_category_listFeiwa(){
...
Tpl::showpage('mb_category.list');
     ...
     * 删除
     */
    public function mb_category_delFeiwa(){
        $lang   = Language::getLangContent();
        if (intval($_GET['gc_id']) > 0){
            ...
            showMessage($lang['link_index_del_succ'],'index.php?app=mb_category&feiwa=mb_category_list');
        }else {
            showMessage($lang['link_index_choose_del'],'index.php?app=mb_category&feiwa=mb_category_list');
        }
    }
     ...
     * 添加
     */
    public function mb_category_addFeiwa(){
        ...
        if ($_POST['form_submit'] == 'ok'){
            ...
            if ($result){
                $url = array(
                    array(
                        'url'=>'index.php?app=mb_category&feiwa=mb_category_add',
                        'msg'=>$lang['link_add_again'],
                    ),
                    array(
                        'url'=>'index.php?app=mb_category&feiwa=mb_category_list',
...
Tpl::showpage('mb_category.add');
     ...
     * 编辑
     */
    public function mb_category_editFeiwa(){
        $lang   = Language::getLangContent();
        $model_link = Model('mb_category');
        if ($_POST['form_submit'] == 'ok'){
            ...
            if ($result){
                ...
                $url = array(
                    array(
                        'url'=>'index.php?app=mb_category&feiwa=mb_category_edit&gc_id='.intval($_POST['gc_id']),
