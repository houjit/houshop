...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title">
      <div class="subject">
        <h3><?php echo $lang['sys_member_predepositmanage'];?></h3>
        <h5><?php echo $lang['sys_member_predepositmanage_subhead'];?></h5>
      </div>
      <ul class="tab-base nc-row">
        ...
        <li><a href="index.php?app=predeposit&feiwa=pd_log_list"><?php echo $lang['sys_member_predepositlog'];?></a></li>
      </ul>
    </div>
  </div>
  <!-- 操作说明 -->
  <div class="explanation" id="explanation">
    <div class="title" id="checkZoom"><i class="fa fa-lightbulb-o"></i>
      ...
      <span id="explanationZoom" title="<?php echo $lang['sys_prompts_span'];?>"></span>
    </div>
    <ul>
  ...
  <div class="ncap-search-bar">
    ...
    <form method="get" name="formSearch" id="formSearch">
      ...
      <div class="bottom"><a href="javascript:void(0);" id="ncsubmit" class="ncap-btn ncap-btn-green mr5">提交查询</a><a href="javascript:void(0);" id="ncreset" class="ncap-btn ncap-btn-orange" title="撤销查询结果，还原列表项所有内容"><i class="fa fa-retweet"></i><?php echo $lang['sys_cancel_search'];?></a></div>
    </form>
  </div>
</div>
...
$(function(){
    ...
    $('#ncsubmit').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?app=predeposit&feiwa=get_xml&'+$("#formSearch").serialize(),query:'',qtype:''}).flexReload();
    });
    // 高级搜索重置
    $('#ncreset').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?app=predeposit&feiwa=get_xml'}).flexReload();
        $("#formSearch")[0].reset();
    });
    
    $("#flexigrid").flexigrid({
        url: 'index.php?app=predeposit&feiwa=get_xml',
        colModel : [
...
function fg_csv(ids) {
    id = ids.join(',');
    window.location.href = $("#flexigrid").flexSimpleSearchQueryString()+'&feiwa=export_step1&id=' + id;
}

function fg_delete(id) {
    if(confirm('删除后将不能恢复，确认删除这项吗？')){
        $.getJSON('index.php?app=predeposit&feiwa=recharge_del', {id:id}, function(data){
            if (data.state) {
