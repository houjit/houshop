...
$(function(){
    $("#flexigrid").flexigrid({
        url: 'index.php?app=store&feiwa=get_reopen_xml',
        colModel : [
...
function reopen_check(id) {
    if(confirm('审核后，系统会自动将店铺的到期时间向后延续\n店铺等级不会自动变更，如果新签约的店铺等级发生变更，请手动更改店铺等级\n确认审核吗？')){
        $.getJSON('index.php?app=store&feiwa=reopen_check', {id:id}, function(data){
            if (data.state) {
                showSucc(data.msg)
...
function reopen_del(id, store_id) {
    if(confirm('删除后将不能恢复，确认删除这项吗？')){
        $.getJSON('index.php?app=store&feiwa=reopen_del', {id:id, store_id:store_id}, function(data){
            if (data.state) {
                $("#flexigrid").flexReload();
            } else {
                showError(data.msg)
