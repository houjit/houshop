...
class goods_classControl extends SystemControl{
    private $links = array(
        array('url'=>'app=goods_class&feiwa=goods_class','lang'=>'sys_manage'),
        array('url'=>'app=goods_class&feiwa=goods_class_import','lang'=>'goods_class_index_import'),
        array('url'=>'app=goods_class&feiwa=tag','lang'=>'goods_class_index_tag')
    );
    ...
    public function indexFeiwa() {
        $this->goods_classFeiwa();
    }

    /**
     * 分类管理
     */
    public function goods_classFeiwa(){
     ...
     * 商品分类添加
     */
    public function goods_class_addFeiwa(){
        ...
        if (chksubmit()){
            ...
            if ($error != ''){
                ...
                if ($result){
                    $url = array(
                        array(
                            'url'=>'index.php?app=goods_class&feiwa=goods_class_add&gc_parent_id='.$_POST['gc_parent_id'],
                            'msg'=>$lang['goods_class_add_again'],
                        ),
                        array(
                            'url'=>'index.php?app=goods_class&feiwa=goods_class',
                            'msg'=>$lang['goods_class_add_back_to_list'],
                        )
                    );
                    $this->log(L('feiwa_add,goods_class_index_class').'['.$_POST['gc_name'].']',1);
                    showMessage($lang['sys_common_save_succ'],$url);
                }else {
                    $this->log(L('feiwa_add,goods_class_index_class').'['.$_POST['gc_name'].']',0);
        ...
        if(is_array($type_list) && !empty($type_list)){
            foreach($type_list as $k=>$val){
                ...
                $t_list[$val['class_id']]['name'] = $val['class_name']==''?L('feiwa_default'):$val['class_name'];
            }
        }
        ksort($t_list);
     ...
     * 编辑
     */
    public function goods_class_editFeiwa(){
        Tpl::output('show_type', $this->show_type);
        $lang   = Language::getLangContent();
        $model_class = Model('goods_class');
        ...
        if (chksubmit()){
            ...
            if (!$result){
                $this->log(L('feiwa_edit,goods_class_index_class').'['.$_POST['gc_name'].']',0);
            ...
            $url = array(
                array(
                    'url'=>'index.php?app=goods_class&feiwa=goods_class_edit&gc_id='.intval($_POST['gc_id']),
            ...
            $this->log(L('feiwa_edit,goods_class_index_class').'['.$_POST['gc_name'].']',1);
        ...
        $this->links[] = array('url'=>'app=goods_class&feiwa=goods_class_edit','lang'=>'feiwa_edit');
     ...
     * 分类导入
     */
    public function goods_class_importFeiwa(){
        $lang   = Language::getLangContent();
        $model_class = Model('goods_class');
        //导入
        if (chksubmit()){
            ...
            if (!empty($_FILES['csv']) && !empty($_FILES['csv']['name']) && $file_type == 'csv'){
                ...
                showMessage($lang['sys_common_op_succ'],'index.php?app=goods_class&feiwa=goods_class');
            }else {
     ...
     * 分类导出
     */
    public function goods_class_exportFeiwa(){
            $model_class = Model('goods_class');
     ...
     * 删除分类
     */
    public function goods_class_delFeiwa(){
        if ($_GET['id'] != ''){
            //删除分类
            Model('goods_class')->delGoodsClassByGcIdString($_GET['id']);
            $this->log(L('feiwa_delete,goods_class_index_class') . '[ID:' . $_GET['id'] . ']',1);
     ...
     * 重置TAG
     */
    public function tag_resetFeiwa(){
        $lang   = Language::getLangContent();
        //实例化模型
        $model_class = Model('goods_class');
        $model_class_tag = Model('goods_class_tag');
        ...
        if(!$return){
            showMessage($lang['goods_class_reset_tag_fail'], 'index.php?app=goods_class&feiwa=tag');
        }
        ...
        if(is_array($goods_class) and !empty($goods_class)) {
            ...
            if($return){
                $this->log(L('feiwa_reset').'tag',1);
                showMessage($lang['sys_common_op_succ'], 'index.php?app=goods_class&feiwa=tag');
            }else{
                $this->log(L('feiwa_reset').'tag',0);
                showMessage($lang['sys_common_op_fail'], 'index.php?app=goods_class&feiwa=tag');
            }
        }else{
            $this->log(L('feiwa_reset').'tag',0);
            showMessage($lang['goods_class_reset_tag_fail_no_class'], 'index.php?app=goods_class&feiwa=tag');
        }
    }
     ...
     * 更新TAG名称
     */
    public function tag_updateFeiwa(){
        $lang   = Language::getLangContent();
        $model_class = Model('goods_class');
        $model_class_tag = Model('goods_class_tag');
        ...
        if(is_array($tag_list) && !empty($tag_list)){
            foreach ($tag_list as $val){
                ...
                if(!$return){
                    $this->log(L('feiwa_update').'tag',0);
                    showMessage($lang['sys_common_op_fail'], 'index.php?app=goods_class&feiwa=tag');
                }
            }
            $this->log(L('feiwa_update').'tag',1);
            showMessage($lang['sys_common_op_succ'], 'index.php?app=goods_class&feiwa=tag');
        }else{
            $this->log(L('feiwa_update').'tag',0);
            showMessage($lang['goods_class_update_tag_fail_no_class'], 'index.php?app=goods_class&feiwa=tag');
        }

    }

    public function tag_editFeiwa() {
        $model_class_tag = Model('goods_class_tag');
    ...
    public function nav_editFeiwa() {
        ...
        if(is_array($brand_list) && !empty($brand_list)){
            foreach($brand_list as $k=>$val){
                ...
                $b_list[$val['class_id']]['name'] = $val['brand_class']==''?L('feiwa_default'):$val['brand_class'];
            }
        }
        ksort($b_list);
