...
class ownmallControl extends SystemControl
{
    ...
    public function addFeiwa()
    {
        if (chksubmit())
        {
            ...
            showMessage('操作成功', urlAdminMall('ownmall', 'list'));
            return;
        }
Tpl::setDirquna('mall');
        Tpl::showpage('ownmall.add');
    }
    ...
    public function delFeiwa() {
        ...
        // 删除自营店id缓存
        $model_store->dropCachedOwnMallIds();
...
Tpl::setDirquna('mall');
        Tpl::showpage('ownmall.bind_class');
    }
    ...
    public function bind_class_updateFeiwa()
    {
        $bid = intval($_GET['id']);
        if($bid <= 0) {
    ...
    public function get_xmlFeiwa() {
        ...
        foreach ($store_list as $value) {
            $param = array();
            $operation = "<a class='btn blue' href='index.php?app=ownmall&feiwa=edit&id=".$value['store_id']."'><i class='fa fa-pencil-square-o'></i>编辑</a>";
            if ($value['bind_all_gc'] != 1 || empty($stores_with_goods[$value['store_id']])) {
                ...
                if ($value['bind_all_gc'] != 1) {
                    $operation .= "<li><a href='index.php?app=ownmall&feiwa=bind_class&id=" . $value['store_id'] . "'>编辑经营类目</a></li>";
                }
