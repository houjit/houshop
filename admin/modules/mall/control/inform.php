...
class informControl extends SystemControl{
    private $links = array(
            array('url'=>'app=inform&feiwa=inform_list','text'=>'未处理'),
            array('url'=>'app=inform&feiwa=inform_handled_list','text'=>'已处理'),
            array('url'=>'app=inform&feiwa=inform_subject_type_list','text'=>'类型设置'),
            array('url'=>'app=inform&feiwa=inform_subject_list','text'=>'主题设置')
    );
    public function __construct(){
        parent::__construct();
        Language::read('inform');
        if ($_GET['feiwa'] == 'index') $_GET['feiwa'] = 'inform_list';
     ...
     * 已处理的举报列表
     */
    public function inform_handled_listFeiwa(){
		Tpl::setDirquna('mall');
        Tpl::showpage('inform.list');
    }
    ...
    public function get_xmlFeiwa(){
        ...
        if ($inform_list && is_array($inform_list)) {
            $pic_base_url = UPLOAD_SITE_URL.'/mall/inform/';
            foreach ($inform_list as $k => $v){
                $list = array();
                if ($_GET['type'] == 'waiting') {
                $list['operation'] = "<a class=\"btn orange\" href=\"index.php?app=inform&feiwa=show_handle_page&inform_id={$v['inform_id']}&inform_goods_name={$v['inform_goods_name']}\"><i class=\"fa fa-gavel\"></i>处理</a>";
                } else {
                    $list['operation'] = '--';
                }
                ...
                $list['inform_goods_name'] = "<a class='open' title='{$v['inform_goods_name']}' href='". urlMall('goods', 'index', array('goods_id' => $v['inform_goods_id'])) ."' target='blank'>{$v['inform_goods_name']}</a>";
                $list['inform_pic'] = '';
     ...
     * 举报类型列表
     */
    public function inform_subject_type_listFeiwa() {
     ...
     * 举报主题列表
     */
    public function inform_subject_listFeiwa(){
     ...
     * 添加举报类型页面
     */
    public function inform_subject_type_addFeiwa(){
Tpl::setDirquna('mall');
     ...
     * 保存添加的举报类型
     */
    public function inform_subject_type_saveFeiwa(){
        ...
        if ($error != ''){
            ...
            $this->log(L('feiwa_add,inform_type').'['.$_POST['inform_type_name'].']',1);
            showMessage(Language::get('feiwa_common_save_succ'),'index.php?app=inform&feiwa=inform_subject_type_list');
        }
    }
     ...
     * 删除举报类型,伪删除只是修改标记
     */
    public function inform_subject_type_dropFeiwa(){

        $inform_type_id = $_GET['inform_type_id'];
        $inform_type_id = "'".implode("','", explode(',', $inform_type_id))."'";
        if(empty($inform_type_id)) {
            showMessage(Language::get('param_error'),'index.php?app=inform');
        }
        ...
        $this->log(L('feiwa_del,inform_type').'[ID:'.$_POST['inform_type_id'].']',1);
        showMessage(Language::get('feiwa_common_del_succ'),'index.php?app=inform&feiwa=inform_subject_type_list');
     ...
     * 添加举报主题页面
     */
    public function inform_subject_addFeiwa(){
     ...
     * 保存添加的举报主题
     */
    public function inform_subject_saveFeiwa(){
        ...
        if ($error != ''){
            ...
            $this->log(L('feiwa_add,inform_subject').'['.$input['inform_subject_type_name'].']',1);
            showMessage(Language::get('feiwa_common_save_succ'),'index.php?app=inform&feiwa=inform_subject_list');
        }
    }
     ...
     * 删除举报主题,伪删除只是修改标记
     */
    public function inform_subject_dropFeiwa(){
        ...
        $this->log(L('feiwa_del,inform_subject').'['.$_POST['inform_subject_id'].']',1);
        showMessage(Language::get('feiwa_common_del_succ'),'index.php?app=inform&feiwa=inform_subject_list');
    }

    /*
     * 显示处理举报
     */
    public function show_handle_pageFeiwa() {
     ...
     * 处理举报
     */
    public function inform_handleFeiwa(){
        ...
        if($model_inform->updateInform($update_array,$where_array)) {
            ...
            showMessage(Language::get('feiwa_common_op_succ'),'index.php?app=inform&feiwa=inform_list');
        }
        else {
