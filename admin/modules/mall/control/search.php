...
class searchControl extends SystemControl{
    ...
    private $_links = array(
        array('url'=>'app=search&feiwa=index','text'=>'默认搜索'),
        array('url'=>'app=search&feiwa=hot','text'=>'热门搜索')
    );
    ...
    public function hot_addFeiwa() {
        ...
        if (chksubmit()) {
            if (count($search_list) >= 10) {
                showMessage('最多可设置10个热搜词','index.php?app=search&feiwa=hot');
            }
            ...
            if ($result){
                showMessage('保存成功','index.php?app=search&feiwa=hot');
            }else {
                showMessage('保存失败');
            }
        }
    ...
    public function hot_editFeiwa() {
        ...
        if (!chksubmit()) {
            ...
            $result = $model_setting->updateSetting(array('rec_search'=>serialize($search_list)));
            if ($result){
                showMessage('编辑成功','index.php?app=search&feiwa=hot');
            }
            showMessage('编辑失败');
        }
