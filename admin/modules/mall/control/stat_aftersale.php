...
class stat_aftersaleControl extends SystemControl{
    private $links = array(
        array('url'=>'app=stat_aftersale&feiwa=refund','lang'=>'stat_refund'),
        array('url'=>'app=stat_aftersale&feiwa=evalstore','lang'=>'stat_evalstore'),
    );
    ...
    public function __construct(){
        ...
        //处理搜索时间
        if (!isset($this->search_arr['feiwa']) || in_array($this->search_arr['feiwa'],array('refund','refundlist','get_refundlist_xml','get_refund_highcharts'))){
     ...
     * 退款统计
     */
    public function refundlistFeiwa(){
        $where = array();
        $model = Model('stat');
     ...
     * 退款统计
     */
    public function get_refund_highchartsFeiwa(){
        $where = array();
     ...
     * 输出退款统计XML数据
     */
    public function get_refundlist_xmlFeiwa(){
        $where = array();
        $model = Model('stat');
        $searchtime_arr = $model->getStarttimeAndEndtime($this->search_arr);
        $where['add_time'] = array('between',$searchtime_arr);
     ...
     * 店铺动态评分统计
     */
    public function evalstoreFeiwa(){
        //店铺分类
     ...
     * 输出店铺动态评分统计XML数据
     */
    public function get_evalstore_xmlFeiwa(){
        $model = Model('stat');
        $where = array();
        if(intval($_GET['store_class']) > 0){
            $where['sc_id'] = intval($_GET['store_class']);
