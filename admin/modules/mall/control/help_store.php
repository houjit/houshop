...
class help_storeControl extends SystemControl{
    ...
    public function indexFeiwa() {
        $this->help_storeFeiwa();
    }

    /**
     * 帮助列表
     */
    public function help_storeFeiwa() {
    ...
    public function get_xmlFeiwa() {
        ...
        foreach ($help_list as $value) {
            $param = array();
            $param['operation'] = "<a class='btn red' href=\"javascript:void(0);\" onclick=\"fg_del('".$value['help_id']."')\"><i class='fa fa-trash-o'></i>删除</a><a class='btn blue' href='index.php?app=help_store&feiwa=edit_help&help_id=".$value['help_id']."' class='url'><i class='fa fa-pencil-square-o'></i>编辑</a>";
     ...
     * 帮助类型
     */
    public function help_typeFeiwa() {
     ...
     * 输出XML数据
     */
    public function get_type_xmlFeiwa() {
        $model_help = Model('help');
        $condition = array();
        if ($_POST['query'] != '') {
            $condition[$_POST['qtype']] = array('like', '%' . $_POST['query'] . '%');
        ...
        foreach ($type_list as $value) {
            ...
            $operation .= "<a class='btn blue' href='index.php?app=help_store&feiwa=edit_type&type_id=".$value['type_id']."' class='url'><i class='fa fa-pencil-square-o'></i>编辑</a>";
    ...
    public function add_helpFeiwa() {
        ...
        if (chksubmit()) {
            ...
            if ($state) {
                ...
                showMessage(Language::get('feiwa_common_save_succ'),'index.php?app=help_store&feiwa=help_store');
            } else {
    ...
    public function add_typeFeiwa() {
        ...
        if (chksubmit()) {
            ...
            if ($state) {
                ...
                showMessage(Language::get('feiwa_common_save_succ'),'index.php?app=help_store&feiwa=help_type');
            } else {
                showMessage(Language::get('feiwa_common_save_fail'));
            }
        }
     ...
     *
     */
    public function edit_typeFeiwa() {
        $model_help = Model('help');
        $condition = array();
