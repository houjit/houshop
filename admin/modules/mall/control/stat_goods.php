...
class stat_goodsControl extends SystemControl{
    private $links = array(
        array('url'=>'app=stat_goods&feiwa=pricerange','lang'=>'stat_goods_pricerange'),
        array('url'=>'app=stat_goods&feiwa=hotgoods','lang'=>'stat_hotgoods'),
        array('url'=>'app=stat_goods&feiwa=goods_sale','lang'=>'stat_goods_sale'),
    );
    ...
    public function __construct(){
        ...
        //处理搜索时间
        if (!isset($this->search_arr['feiwa']) || in_array($this->search_arr['feiwa'],array('pricerange','hotgoods','goods_sale','get_goods_xml'))){
    ...
    public function indexFeiwa() {
        $this->pricerangeFeiwa();
    }

    /**
     * 价格区间统计
     */
    public function pricerangeFeiwa(){
     ...
     * 热卖商品列表
     */
    public function hotgoods_listFeiwa(){
        $model = Model('stat');
        switch ($_GET['type']){
           case 'goodsnum':
               $sort_text = '下单量';
     ...
     * 商品销售明细
     */
    public function goods_saleFeiwa(){
        if(!$this->search_arr['search_type']){
            $this->search_arr['search_type'] = 'day';
        }
        $model = Model('stat');
