...
//             $list = array();
//             $list['operation'] = "<a class=\"btn green\" href=\"index.php?app=vr_bill&feiwa=show_statis&os_month={$bill_info['os_month']}\"><i class=\"fa fa-list-alt\"></i>查看</a>";
...
//     }
    ...
    public function get_bill_xmlFeiwa(){
        ...
        foreach ($bill_list as $bill_info) {
            ...
            if (in_array($bill_info['ob_state'],array(2,3))) {
                $list['operation'] = "<a class=\"btn orange\" href=\"index.php?app=vr_bill&feiwa=show_bill&ob_id={$bill_info['ob_id']}\"><i class=\"fa fa-gavel\"></i>处理</a>";
            } else {
                $list['operation'] = "<a class=\"btn green\" href=\"index.php?app=vr_bill&feiwa=show_bill&ob_id={$bill_info['ob_id']}\"><i class=\"fa fa-list-alt\"></i>查看</a>";
            }
    ...
    public function get_bill_info_code_xmlFeiwa(){
        $ob_id = intval($_GET['ob_id']);
        if ($ob_id <= 0) {
            exit();
        }
        ...
        foreach ($code_list as $code_info) {
            $list = array();
            $list['operation'] = "<a class=\"btn green\" href=\"index.php?app=vr_order&feiwa=show_order&order_id={$code_info['order_id']}\"><i class=\"fa fa-list-alt\"></i>查看</a>";
    ...
    public function bill_payFeiwa(){
        ...
        if (chksubmit()){
            ...
            if ($update){
                ...
                showMessage('保存成功','index.php?app=vr_bill');
            }else{
                $this->log('账单付款,账单号：'.$ob_id,1);
    ...
    public function export_orderFeiwa(){
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){
                ...
                Tpl::output('list',$array);
                Tpl::output('murl','index.php?app=vr_bill&feiwa=show_bill&ob_id='.$ob_id);
				Tpl::setDirquna('mall');
                Tpl::showpage('export.excel');
                exit();
            }else{
