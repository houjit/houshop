...
class complainControl extends SystemControl {
    ...
    private $links = array(
            array('url'=>'app=complain&feiwa=complain_new_list','text'=>'新投诉'),
            array('url'=>'app=complain&feiwa=complain_appeal_list','text'=>'待申诉'),
            array('url'=>'app=complain&feiwa=complain_talk_list','text'=>'对话中'),
            array('url'=>'app=complain&feiwa=complain_handle_list','text'=>'待仲裁'),
            array('url'=>'app=complain&feiwa=complain_finish_list','text'=>'已关闭'),
            array('url'=>'app=complain&feiwa=complain_subject_list','text'=>'主题设置'),
            array('url'=>'app=complain&feiwa=complain_setting','text'=>'时效设置')
    );
    public function __construct() {
        parent::__construct();
        Language::read('complain');
        if ($_GET['feiwa'] == 'index') $_GET['feiwa'] = 'complain_new_list';
    ...
    public function indexFeiwa() {
        $this->complain_new_listFeiwa();
    }

    /*
     * 未处理的投诉列表
     */
    public function complain_new_listFeiwa() {
     ...
     * 待申诉的投诉列表
     */
    public function complain_appeal_listFeiwa() {
     ...
     * 对话的投诉列表
     */
    public function complain_talk_listFeiwa() {
     ...
     * 待仲裁的投诉列表
     */
    public function complain_handle_listFeiwa() {
     ...
     * 已关闭的投诉列表
     */
    public function complain_finish_listFeiwa() {
     ...
     * 获取投诉列表
    */
    public function get_new_xmlFeiwa() {
        $page = new Page();
        ...
        foreach ($complain_list as $k => $complain_info) {
            $list = array();
            if ($_GET['state'] == 99) {
                $list['operation'] = "<a class=\"btn orange\" href=\"index.php?app=complain&feiwa=complain_progress&complain_id={$complain_info['complain_id']}\"><i class=\"fa fa-list-alt\"></i>查看</a>";
            } else {
                $list['operation'] = "<a class=\"btn orange\" href=\"index.php?app=complain&feiwa=complain_progress&complain_id={$complain_info['complain_id']}\"><i class=\"fa fa-gavel\"></i>处理</a>";
            }
            ...
            $list['accused_name'] = "<a class='open' title='{$complain_info['accused_name']}' href='". urlMall('show_store','index', array('store_id'=>$complain_info['accused_id'])) ."' target='blank'>{$complain_info['accused_name']}</a>";
     ...
     * 进行中的投诉
     */
    public function complain_progressFeiwa() {
     ...
     * 审核提交的投诉
     */
    public function complain_verifyFeiwa() {
        ...
        if(intval($complain_info['complain_state']) === self::STATE_NEW) {
            ...
            if($model_complain->updateComplain($update_array,$where_array)) {
                ...
                showMessage(Language::get('complain_verify_success'),'index.php?app=complain&feiwa=complain_new_list');
            } else {
                showMessage(Language::get('complain_verify_fail'),'index.php?app=complain&feiwa=complain_new_list');
            }
        } else {
     ...
     * 关闭投诉
     */
    public function complain_closeFeiwa() {
        //获取输入的数据
        $complain_id = intval($_POST['complain_id']);
     ...
     * 投诉主题列表
     */
    public function complain_subject_listFeiwa() {
        /*
         * 实例化分页
         */
        $page = new Page();
     ...
     * 添加投诉主题页面
     */
    public function complain_subject_addFeiwa() {
     ...
     * 保存添加的投诉主题
     */
    public function complain_subject_saveFeiwa() {
        //获取提交的内容
        ...
        if ($error != '') {
            ...
            showMessage(Language::get('complain_subject_add_success'),'index.php?app=complain&feiwa=complain_subject_list');
        }
    }
     ...
     * 删除投诉主题,伪删除只是修改标记
     */
    public function complain_subject_dropFeiwa() {
        $complain_subject_id = trim($_GET['complain_subject_id']);
        if(empty($complain_subject_id)) {
            showMessage(Language::get('param_error'),'index.php?app=complain');
        }
        $model_complain_subject= Model('complain_subject');
        $update_array = array();
        ...
        if($model_complain_subject->updateComplainSubject($update_array,$where_array)) {
            ...
            showMessage(Language::get('complain_subject_delete_success'),'index.php?app=complain&feiwa=complain_subject_list');
        } else {
            showMessage(Language::get('complain_subject_delete_fail'),'index.php?app=complain&feiwa=complain_subject_list');
        }
    }
     ...
     * 根据投诉id获取投诉对话列表
     */
    public function get_complain_talkFeiwa() {
        $complain_id = intval($_POST['complain_id']);
        $complain_info = $this->get_complain_info($complain_id);
     ...
     * 发布投诉对话
     */
    public function publish_complain_talkFeiwa() {
        $complain_id = intval($_POST['complain_id']);
     ...
     * 屏蔽对话
     */
    public function forbit_talkFeiwa() {
     ...
     * 投诉设置
     **/
    public function complain_settingFeiwa() {
        //读取设置内容 $list_setting
     ...
     * 投诉设置保存
     **/
    public function complain_setting_saveFeiwa() {
        $model_setting = Model('setting');
    ...
    private function get_complain_state_link($complain_state) {
        switch(intval($complain_state)) {
            case self::STATE_NEW:
                return 'index.php?app=complain&feiwa=complain_new_list';
                break;
            case self::STATE_APPEAL:
                return 'index.php?app=complain&feiwa=complain_appeal_list';
                break;
            case self::STATE_TALK:
                return 'index.php?app=complain&feiwa=complain_talk_list';
                break;
            case self::STATE_HANDLE:
                return 'index.php?app=complain&feiwa=complain_handle_list';
                break;
            case self::STATE_FINISH:
                return 'index.php?app=complain&feiwa=complain_finish_list';
                break;
            default:
                showMessage(Language::get('param_error'));
        }
    ...
    private function show_menu($menu_key) {
        $menu_array = array(
            'complain_new_list'=>array('menu_type'=>'link','menu_name'=>Language::get('complain_new_list'),'menu_url'=>'index.php?app=complain&feiwa=complain_new_list'),
            'complain_appeal_list'=>array('menu_type'=>'link','menu_name'=>Language::get('complain_appeal_list'),'menu_url'=>'index.php?app=complain&feiwa=complain_appeal_list'),
            'complain_talk_list'=>array('menu_type'=>'link','menu_name'=>Language::get('complain_talk_list'),'menu_url'=>'index.php?app=complain&feiwa=complain_talk_list'),
            'complain_handle_list'=>array('menu_type'=>'link','menu_name'=>Language::get('complain_handle_list'),'menu_url'=>'index.php?app=complain&feiwa=complain_handle_list'),
            'complain_finish_list'=>array('menu_type'=>'link','menu_name'=>Language::get('complain_finish_list'),'menu_url'=>'index.php?app=complain&feiwa=complain_finish_list'),
            'complain_subject_list'=>array('menu_type'=>'link','menu_name'=>Language::get('complain_subject_list'),'menu_url'=>'index.php?app=complain&feiwa=complain_subject_list'),
            'complain_subject_add'=>array('menu_type'=>'link','menu_name'=>Language::get('complain_subject_add'),'menu_url'=>'index.php?app=complain&feiwa=complain_subject_add'),
            'complain_setting'=>array('menu_type'=>'link','menu_name'=>Language::get('complain_setting'),'menu_url'=>'index.php?app=complain&feiwa=complain_setting'),
