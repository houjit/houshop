...
class stat_marketingControl extends SystemControl{
    private $links = array(
        array('url'=>'app=stat_marketing&feiwa=promotion','lang'=>'stat_promotion'),
        array('url'=>'app=stat_marketing&feiwa=group','lang'=>'stat_group'),
    );
    ...
    public function __construct(){
        ...
        //处理搜索时间
        if (!isset($this->search_arr['feiwa']) || in_array($this->search_arr['feiwa'],array('promotion','group','get_groupgoods_xml','groupgoods','get_grouplist_xml'))){
     ...
     * 促销分析
     */
    public function promotionFeiwa(){
     ...
     * 促销销售趋势分析
     */
    public function promotiontrendFeiwa(){
        //优惠类型数组
        $goodstype_arr = array(2=>'团购',3=>'限时折扣',4=>'优惠套装');
     ...
     * 输出团购统计XML数据
     */
    public function get_grouplist_xmlFeiwa(){
        ...
        if (!empty($grouplist) && is_array($grouplist)){
            ...
            foreach ($grouplist as $k => $v){
                ...
                $out_array['groupbuy_name'] = '<a href="'.urlMall('show_groupbuy', 'groupbuy_detail', array('group_id' => $v['groupbuy_id'])).'" target="_blank">'.$v['groupbuy_name'].'</a>';
                $out_array['start_time'] = date('Y-m-d',$v['start_time']);
                $out_array['end_time'] = date('Y-m-d',$v['end_time']);
                $out_array['goods_name'] = '<a href="'.urlMall('goods', 'index', array('goods_id' => $v['goods_id'])).'" target="_blank">'.$v['goods_name'].'</a>';
     ...
     * 团购商品统计
     */
    public function groupgoodsFeiwa(){
        $model = Model('stat');
        $where = array();
        //获得搜索的开始时间和结束时间
        $searchtime_arr = $model->getStarttimeAndEndtime($this->search_arr);
        ...
        foreach ((array)$statlist_tmp as $k => $v){
            ...
            foreach ($statheader as $h_k=>$h_v){
                ...
                if ($h_v['key'] == 'goods_name'){
                    $tmp[$h_v['key']] = '<a href="'.urlMall('goods', 'index', array('goods_id' => $v['goods_id'])).'" target="_blank">'.$v['goods_name'].'</a>';
                }
            }
            $statlist[] = $tmp;
        }
     ...
     * 输出团购商品统计XML数据
     */
    public function get_groupgoods_xmlFeiwa(){
        $model = Model('stat');
        $where = array();
        //获得搜索的开始时间和结束时间
        $searchtime_arr = $model->getStarttimeAndEndtime($this->search_arr);
