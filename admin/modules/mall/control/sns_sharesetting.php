...
class sns_sharesettingControl extends SystemControl{
    ...
    public function indexFeiwa() {
        $this->sharesettingFeiwa();
    }

    /**
     * 绑定接口列表
     */
    public function sharesettingFeiwa(){
    ...
    public function setFeiwa(){
        ...
        if ($result){
            $this->log(L('feiwa_edit,feiwa_binding_manage'),null);
    ...
    public function editFeiwa(){
        ...
        if(chksubmit()){
            ...
            if ($result){
                $this->log(L('feiwa_edit,feiwa_binding_manage'),null);
                showMessage(Language::get('feiwa_common_save_succ'),'index.php?app=sns_sharesetting&feiwa=sharesetting');
            }else {
