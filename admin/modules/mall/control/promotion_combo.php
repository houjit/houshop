...
class promotion_comboControl extends SystemControl{
    private $links = array(
        array('url'=>'app=promotion_combo&feiwa=index','text'=>'商品列表'),
        array('url'=>'app=promotion_combo&feiwa=combo_quota_list','text'=>'套餐列表'),
        array('url'=>'app=promotion_combo&feiwa=combo_setting','text'=>'设置')
    );
    public function __construct(){
        ...
        if (intval($_GET['promotion_allow']) !== 1 && intval(C('promotion_allow')) !== 1){
            $url = array(
                ...
                array(
                    'url'=>'index.php?app=promotion_combo&promotion_allow=1',
                    'msg'=>L('open'),
                )
            );
    ...
    public function indexFeiwa() {
        ...
        $this->combo_goods_listFeiwa();
    }

    /**
     * 活动商品列表
     */
    public function combo_goods_listFeiwa() {
     ...
     * 活动商品管理XML
     */
    public function get_combo_goods_xmlFeiwa() {
        $condition = array();
        if ($_POST['query'] != '') {
        ...
        $flipped_own_mall_ids = array_flip(Model('store')->getOwnMallIds());
        ...
        foreach ($goods_list as $value) {
            $param = array();
            $operation = "<a class='btn red' href='javascript:;' onclick=\"fg_del('" . $value['goods_id'] . "')\"><i class='fa fa-trash-o'></i>删除</a><a class='btn green' target='_blank' href='" . urlMall('goods', 'index', array('goods_id' => $value['goods_id'])) . "'><i class='fa fa-list-alt'></i>查看</a>";
            ...
            if (isset($flipped_own_mall_ids[$value['store_id']])) {
     ...
     * 删除订金预售商品活动
     */
    public function del_combo_goodsFeiwa() {
        $id = intval($_GET['id']);
        if ($id > 0) {
     ...
     * 套餐列表
     */
    public function combo_quota_listFeiwa() {
     ...
     * 设置
     */
    public function combo_settingFeiwa() {
        // 实例化模型
        $model_setting = Model('setting');
