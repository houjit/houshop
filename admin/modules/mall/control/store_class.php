...
class store_classControl extends SystemControl{
    ...
    public function indexFeiwa() {
        $this->store_classFeiwa();
    }

    /**
     * 店铺分类
     */
    public function store_classFeiwa(){
        ...
        if (chksubmit()){
            if (!empty($_POST['check_sc_id']) && is_array($_POST['check_sc_id']) ){
                ...
                if ($result) {
                    $this->log(L('feiwa_del,store_class').'[ID:'.implode(',',$_POST['check_sc_id']).']',1);
     ...
     * 商品分类添加
     */
    public function store_class_addFeiwa(){
        $lang   = Language::getLangContent();
        $model_class = Model('store_class');
        if (chksubmit()){
            //验证
            ...
            if ($error != ''){
                ...
                if ($result){
                    $url = array(
                    array(
                    'url'=>'index.php?app=store_class&feiwa=store_class_add',
                    'msg'=>$lang['continue_add_store_class'],
                    ),
                    array(
                    'url'=>'index.php?app=store_class&feiwa=store_class',
                    'msg'=>$lang['back_store_class_list'],
                    )
                    );
                    $this->log(L('feiwa_add,store_class').'['.$_POST['sc_name'].']',1);
                    showMessage($lang['sys_common_save_succ'],$url,'html','succ',1,5000);
                }else {
     ...
     * 编辑
     */
    public function store_class_editFeiwa(){
        $lang   = Language::getLangContent();
        ...
        if (chksubmit()){
            ...
            if ($error != ''){
                ...
                if ($result){
                    $this->log(L('feiwa_edit,store_class').'['.$_POST['sc_name'].']',1);
                    showMessage($lang['sys_common_save_succ'],'index.php?app=store_class&feiwa=store_class');
                }else {
                    showMessage($lang['sys_common_save_fail']);
                }
            }
     ...
     * 删除分类
     */
    public function store_class_delFeiwa(){
        $lang   = Language::getLangContent();
        $model_class = Model('store_class');
        if (intval($_GET['sc_id']) > 0){
            ...
            if ($result) {
                 $this->log(L('feiwa_del,store_class').'[ID:'.$_GET['sc_id'].']',1);
                 showMessage($lang['sys_common_del_succ'],getReferer());
            }
        }
        showMessage($lang['sys_common_del_fail'],'index.php?app=store_class&feiwa=store_class');
    }
