...
class storeControl extends SystemControl{
    ...
    private $_links = array(
        array('url'=>'app=store&feiwa=store','text'=>'管理'),
        array('url'=>'app=store&feiwa=store_joinin','text'=>'开店申请'),
        array('url'=>'app=store&feiwa=reopen_list','text'=>'续签申请'),
        array('url'=>'app=store&feiwa=store_bind_class_applay_list','text'=>'经营类目申请'),
        array('url'=>'app=store&feiwa=bill_cycle','text'=>'结算周期设置')
    );
     ...
     * 店铺结算周期
     */
    public function bill_cycleFeiwa(){
    ...
    public function get_xmlFeiwa() {
        ...
        foreach ($store_list as $value) {
            ...
            $operation = "<a class='btn green' href='index.php?app=store&feiwa=store_joinin_detail&member_id=".$value['member_id']."'><i class='fa fa-list-alt'></i>查看</a><span class='btn'><em><i class='fa fa-cog'></i>" . L('feiwa_set') . " <i class='arrow'></i></em><ul><li><a href='index.php?app=store&feiwa=store_edit&store_id=" . $value['store_id'] . "'>编辑店铺信息</a></li><li><a href='index.php?app=store&feiwa=store_bind_class&store_id=" . $value['store_id'] . "'>修改经营类目</a></li>";
     ...
     * 输出XML数据
     */
    public function get_bill_cycle_xmlFeiwa() {
        $model_store = Model('store');
        $condition = array();
        if ($_GET['store_name'] != '') {
            $condition['store_name'] = array('like', '%' . $_GET['store_name'] . '%');
        ...
        foreach ($store_list as $value) {
            $param = array();
            $store_state = $this->getStoreState($value);
            $operation = "<a class='btn blue' href='index.php?app=store&feiwa=bill_cycyle_edit&store_id=".$value['store_id']."'><i class='fa fa-pencil-square-o'></i>编辑</a>";
            $operation .= "</ul></span>";
            $param['operation'] = $operation;
            $param['store_id'] = $value['store_id'];
            $store_name = "<a class='" . $store_state . "' href='". urlMall('show_store', 'index', array('store_id' => $value['store_id'])) ."' target='blank'>";
    ...
    public function export_csvFeiwa() {
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){   //显示下载链接
                ...
                Tpl::output('murl','index.php?app=store&feiwa=index');
				Tpl::setDirquna('mall');
    ...
    public function store_editFeiwa(){
        ...
        if (chksubmit()){
            ...
            if ($result){
                $url = array(
                array(
                'url'=>'index.php?app=store&feiwa=store',
                'msg'=>$lang['back_store_list'],
                ),
                array(
                'url'=>'index.php?app=store&feiwa=store_edit&store_id='.intval($_POST['store_id']),
                'msg'=>$lang['countinue_add_store'],
                ),
                );
                $this->log(L('feiwa_edit,store').'['.$_POST['store_name'].']',1);
                showMessage($lang['sys_common_save_succ'],$url);
            }else {
                $this->log(L('feiwa_edit,store').'['.$_POST['store_name'].']',1);
                showMessage($lang['sys_common_save_fail']);
            }
        }
        //取店铺信息
     ...
     * 店铺结算周期编辑
     */
    public function bill_cycyle_editFeiwa(){
        $lang = Language::getLangContent();
        ...
        if (chksubmit()){
            ...
            if ($result){
                ...
                showMessage($lang['sys_common_save_succ'],'index.php?app=store&feiwa=bill_cycle');
            }else {
                $this->log('设置店铺结算周期['.$_POST['store_name'].']',1);
                showMessage($lang['sys_common_save_fail'],'index.php?app=store&feiwa=bill_cycle');
            }
        }
     ...
     * 编辑保存注册信息
     */
    public function edit_save_joininFeiwa() {
        if (chksubmit()) {
            ...
            if ($result) {
                showMessage(L('feiwa_common_op_succ'), 'index.php?app=store&feiwa=store');
            } else {
     ...
     * 店铺经营类目管理
     */
    public function store_bind_classFeiwa() {
     ...
     * 添加经营类目
     */
    public function store_bind_class_addFeiwa() {
     ...
     * 删除经营类目
     */
    public function store_bind_class_delFeiwa() {
    ...
    public function store_bind_class_updateFeiwa() {
        $bid = intval($_GET['id']);
        if($bid <= 0) {
        ...
        if ($new_commis_rate < 0 || $new_commis_rate >= 100) {
            ...
            if($result) {
                ...
                echo json_encode(array('result'=>FALSE,'message'=>L('feiwa_common_op_fail')));
                die;
            }
        }
    }

    public function feiwa_addFeiwa()
    {
        if (chksubmit())
        {
            ...
            showMessage('操作成功','index.php?app=store&feiwa=store');
            return;
        }
		Tpl::setDirquna('mall');
	 ...
	 public function check_seller_nameFeiwa()
    {
    ...
    public function check_member_nameFeiwa()
    {
     ...
     * 店铺 待审核列表
     */
    public function store_joininFeiwa(){
     ...
     * 输出XML数据
     */
    public function get_joinin_xmlFeiwa() {
        ...
        foreach ($store_list as $value) {
            ...
            if(in_array(intval($value['joinin_state']), array(STORE_JOIN_STATE_NEW, STORE_JOIN_STATE_PAY))) {
                $operation = "<a class='btn orange' href=\"index.php?app=store&feiwa=store_joinin_detail&member_id=". $value['member_id'] ."\"><i class=\"fa fa-check-circle\"></i>审核</a>";
            } else {
                $operation = "<a class='btn green' href=\"index.php?app=store&feiwa=store_joinin_detail&member_id=". $value['member_id'] ."\"><i class=\"fa fa-list-alt\"></i>查看</a>";
            }
            $param['operation'] = $operation;
     ...
     * 经营类目申请列表
     */
    public function store_bind_class_applay_listFeiwa(){
     ...
     * 输出XML数据
     */
    public function get_bind_class_applay_xmlFeiwa() {
        $model_store_bind_class = Model('store_bind_class');
        // 设置页码参数名称
        $condition = array();
        ...
        foreach ($store_bind_class_list as $value) {
            $param = array();
            if($value['state'] == '0') {
                $operation = "<a class='btn orange' href=\"javascript:if(confirm('确认审核吗？'))window.location = 'index.php?app=store&feiwa=store_bind_class_applay_check&bid=".$value['bid']."&store_id=".$value['store_id']."'\"><i class=\"fa fa-check-circle\"></i>审核</a>";
            } else {
                $operation = "<a class='btn red' href=\"javascript:if(confirm('".($value['state'] == '1' ? '该类目已经审核通过，删除它可能影响到商家的使用，' : null)."确认删除吗？'))window.location = 'index.php?app=store&feiwa=store_bind_class_applay_del&bid=".$value['bid']."&store_id=".$value['store_id']."'\"><i class=\"fa fa-trash-o\"></i>删除</a>";
            }
            $param['operation'] = $operation;
            $param['store_id'] = $value['store_id'];
     ...
     * 审核经营类目申请
     */
    public function store_bind_class_applay_checkFeiwa() {
        $model_store_bind_class = Model('store_bind_class');
        $condition = array();
     ...
     * 删除经营类目申请
     */
    public function store_bind_class_applay_delFeiwa() {
        $model_store_bind_class = Model('store_bind_class');
        $condition = array();
        $condition['bid'] = intval($_GET['bid']);
     ...
     * 店铺续签申请列表
     */
    public function reopen_listFeiwa(){
     ...
     * 输出XML数据
     */
    public function get_reopen_xmlFeiwa() {
     ...
     * 审核店铺续签申请
     */
    public function reopen_checkFeiwa() {
        $id = intval($_GET['id']);
        if ($id > 0) {
            $model_store_reopen = Model('store_reopen');
            $condition = array();
     ...
     * 删除店铺续签申请
     */
    public function reopen_delFeiwa() {
        $id = intval($_GET['id']);
        if ($id > 0) {
            $model_store_reopen = Model('store_reopen');
            $condition = array();
     ...
     * 审核详细页
     */
    public function store_joinin_detailFeiwa(){
        $model_store_joinin = Model('store_joinin');
     ...
     * 审核
     */
    public function store_joinin_verifyFeiwa() {
        $model_store_joinin = Model('store_joinin');
    ...
    private function store_joinin_verify_pass($joinin_detail) {
        ...
        if ($param['paying_amount'] > 0) {
            showMessage('店铺入驻申请审核完成','index.php?app=store&feiwa=store_joinin');
        } else {
    ...
    private function store_joinin_verify_open($joinin_detail) {
        ...
        if($_POST['verify_type'] === 'pass') {
            ...
            if($state) {
                ...
                showMessage('店铺开店成功','index.php?app=store&feiwa=store_joinin');
            } else {
                showMessage('店铺开店失败','index.php?app=store&feiwa=store_joinin');
            }
        } else {
            showMessage('店铺开店拒绝','index.php?app=store&feiwa=store_joinin');
        }
    }
     ...
     * 提醒续费
     */
    public function remind_renewalFeiwa() {
        $store_id = intval($_GET['store_id']);
     ...
     * 验证店铺名称是否存在
     */
    public function ckeck_store_nameFeiwa() {
        /**
         * 实例化商家模型
         */
        $where = array();
