...
class store_gradeControl extends SystemControl{
    ...
    public function indexFeiwa() {
        $this->store_gradeFeiwa();
    }

    /**
     * 店铺等级
     */
    public function store_gradeFeiwa(){
        /**
         * 读取语言包
         */
            ...
            if (!empty($_POST['check_sg_id'])){
                if (is_array($_POST['check_sg_id'])){
                    ...
                    foreach ($_POST['check_sg_id'] as $k => $v){
                        ...
                        //判断是否默认等级，默认等级不能删除
                        if ($v == 1){
                            //showMessage('默认等级不能删除 ','index.php?app=store_grade&feiwa=store_grade');
                            showMessage($lang['default_store_grade_no_del'],'index.php?app=store_grade&feiwa=store_grade');
                        }
                ...
                dkcache('store_grade');
                $this->log(L('feiwa_del,store_grade').'[ID:'.implode(',',$_POST['check_sg_id']).']',1);
                showMessage($lang['sys_common_del_succ']);
            }else {
                showMessage($lang['sys_common_del_fail']);
            }
        }
     ...
     * 新增等级
     */
    public function store_grade_addFeiwa(){
        $lang   = Language::getLangContent();
        ...
        if (chksubmit()){
            ...
            if ($error != ''){
                ...
                if ($result){
                    dkcache('store_grade');
                    $this->log(L('feiwa_add,store_grade').'['.$_POST['sg_name'].']',1);
                    showMessage($lang['sys_common_save_succ'],'index.php?app=store_grade&feiwa=store_grade');
                }else {
     ...
     * 等级编辑
     */
    public function store_grade_editFeiwa(){
        $lang   = Language::getLangContent();

        $model_grade = Model('store_grade');
        if (chksubmit()){
            if (!$_POST['sg_id']){
                showMessage($lang['grade_parameter_error'],'index.php?app=store_grade&feiwa=store_grade');
            }
            /**
             * 验证
             */
            ...
            if ($error != ''){
                ...
                if (!$this->checkGradeName(array('sg_name'=>trim($_POST['sg_name']),'sg_id'=>intval($_POST['sg_id'])))){
                    showMessage($lang['now_store_grade_name_is_there'],'index.php?app=store_grade&feiwa=store_grade_edit&sg_id='.intval($_POST['sg_id']));
                }
                //验证级别是否存在
                if (!$this->checkGradeSort(array('sg_sort'=>trim($_POST['sg_sort']),'sg_id'=>intval($_POST['sg_id'])))){
                    showMessage($lang['add_gradesortexist'],'index.php?app=store_grade&feiwa=store_grade_edit&sg_id='.intval($_POST['sg_id']));
                }
                $update_array = array();
                ...
                if ($result){
                    dkcache('store_grade');
                    $this->log(L('feiwa_edit,store_grade').'['.$_POST['sg_name'].']',1);
                    showMessage($lang['sys_common_save_succ']);
                }else {
                    showMessage($lang['sys_common_save_fail']);
                }
            }
     ...
     * 删除等级
     */
    public function store_grade_delFeiwa(){
        /**
         * 读取语言包
         */
        $lang   = Language::getLangContent();
        ...
        if (intval($_GET['sg_id']) > 0){
            ...
            if (!$this->isable_delGrade(intval($_GET['sg_id']))){
                showMessage($lang['del_gradehavestore'],'index.php?app=store_grade&feiwa=store_grade');
            }
            /**
             * 删除分类
             */
            ...
            dkcache('store_grade');
            $this->log(L('feiwa_del,store_grade').'[ID:'.intval($_GET['sg_id']).']',1);
            showMessage($lang['sys_common_del_succ'],'index.php?app=store_grade&feiwa=store_grade');
        }else {
            showMessage($lang['sys_common_del_fail'],'index.php?app=store_grade&feiwa=store_grade');
        }
    }
     ...
     * 等级：设置可选模板
     */
    public function store_grade_templatesFeiwa(){
        /**
         * 读取语言包
         */
        $lang   = Language::getLangContent();
        ...
        if (chksubmit()){
            ...
            if ($result){
                $this->log(L('feiwa_edit,store_grade_tpl'),1);
                showMessage($lang['sys_common_save_succ'],'index.php?app=store_grade&feiwa=store_grade');
            }else {
                showMessage($lang['sys_common_save_fail']);
            }
        ...
        //主题配置信息
        $style_data = array();
        $style_configurl = BASE_ROOT_PATH.DS.DIR_MALL.'/templates/'.TPL_MALL_NAME.DS.'store'.DS.'style'.DS."styleconfig.php";
