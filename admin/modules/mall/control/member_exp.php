...
class member_expControl extends SystemControl{
     ...
     * 设置经验值获取规则
     */
    public function expsettingFeiwa(){
        ...
        if (chksubmit()){
            ...
            if ($result === true){
                $this->log(L('feiwa_edit,feiwa_exppoints_manage,feiwa_exppoints_setting'),1);
     ...
     * 会员管理
     */
    public function member_gradeFeiwa(){
        $model_setting = Model('setting');
        $list_setting = $model_setting->getListSetting();
        ...
        if (chksubmit()){
            ...
            if ($result){
                $this->log(L('feiwa_edit,feiwa_member_grade'),1);
                showDialog(L('feiwa_common_save_succ'),'reload','succ');
            } else {
                $this->log(L('feiwa_edit,feiwa_member_grade'),0);
