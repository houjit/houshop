...
class refundControl extends SystemControl{
    ...
    private $links = array(
            array('url'=>'app=refund','text'=>'待处理'),
            array('url'=>'app=refund&feiwa=refund_all','text'=>'所有记录'),
            array('url'=>'app=refund&feiwa=reason','text'=>'退款退货原因')
    );
    ...
    public function get_manage_xmlFeiwa() {
        ...
        foreach ($refund_list as $k => $refund_info) {
            $list = array();
            $list['operation'] = "<a class=\"btn orange\" href=\"index.php?app=refund&feiwa=edit&refund_id={$refund_info['refund_id']}\"><i class=\"fa fa-gavel\"></i>处理</a>";
            ...
            if ($refund_info['goods_id'] > 0) {
                $list['goods_name'] = "<a class='open' title='{$refund_info['goods_name']}' href='". urlMall('goods', 'index', array('goods_id' => $refund_info['goods_id'])) .
     ...
     * 所有记录
     */
    public function refund_allFeiwa() {
		Tpl::setDirquna('mall');
    ...
    public function editFeiwa() {
        ...
        if (chksubmit()) {
            ...
            if ($state) {
                ...
                showMessage(Language::get('feiwa_common_save_succ'),'index.php?app=refund&feiwa=index');
            } else {
                showMessage(Language::get('feiwa_common_save_fail'));
            }
        }
     ...
     *
     */
    public function add_reasonFeiwa() {
        $model_refund = Model('refund_return');
        if (chksubmit()) {
            $reason_array = array();
            ...
            if ($state) {
                ...
                showMessage(Language::get('feiwa_common_save_succ'),'index.php?app=refund&feiwa=reason');
            } else {
                showMessage(Language::get('feiwa_common_save_fail'));
            }
        }
     ...
     *
     */
    public function edit_reasonFeiwa() {
        $model_refund = Model('refund_return');
        $condition = array();
     ...
     *
     */
    public function del_reasonFeiwa() {
        $model_refund = Model('refund_return');
        $condition = array();
        $condition['reason_id'] = intval($_GET['reason_id']);
        ...
        if ($state) {
            ...
            showMessage(Language::get('feiwa_common_del_succ'),'index.php?app=refund&feiwa=reason');
        } else {
