...
class vr_refundControl extends SystemControl{
    ...
    private $links = array(
            array('url'=>'app=vr_refund&feiwa=refund_manage','text'=>'待处理'),
            array('url'=>'app=vr_refund&feiwa=refund_all','text'=>'所有记录'),
    );
    ...
    public function indexFeiwa() {
        $this->refund_manageFeiwa();
    }

    /**
     * 待处理列表
     */
    public function refund_manageFeiwa() {
        $_GET['waiting'] = 1;
        $this->refund_allFeiwa();
    }
    ...
    public function get_all_xmlFeiwa() {
        ...
        foreach ($refund_list as $k => $refund_info) {
            ...
            if ($_GET['waiting'] == 1 && $refund_info['admin_state'] == 1) {
                $list['operation'] = "<a class=\"btn orange\" href=\"index.php?app=vr_refund&feiwa=edit&refund_id={$refund_info['refund_id']}\"><i class=\"fa fa-gavel\"></i>处理</a>";
            } else {
                $list['operation'] = "<a class=\"btn green\" href=\"index.php?app=vr_refund&feiwa=view&refund_id={$refund_info['refund_id']}\"><i class=\"fa fa-list-alt\"></i>查看</a>";
            }
            ...
            $list['goods_name'] = "<a class='open' title='{$refund_info['goods_name']}' href='". urlMall('goods', 'index', array('goods_id' => $refund_info['goods_id'])) ."' target='blank'>{$refund_info['goods_name']}</a>";
    ...
    public function editFeiwa() {
        ...
        if (chksubmit()) {
            ...
            if ($state) {
                ...
                $param['param'] = array(
                    'refund_url' => urlMall('member_vr_refund', 'view', array('refund_id' => $refund['refund_id'])),
                    'refund_sn' => $refund['refund_sn']
                );
                QueueClient::push('sendMemberMsg', $param);

                $this->log('虚拟订单退款审核，退款编号'.$refund['refund_sn']);
                showMessage(Language::get('feiwa_common_save_succ'),'index.php?app=vr_refund&feiwa=refund_manage');
            } else {
                showMessage(Language::get('feiwa_common_save_fail'));
            }
        }
