...
class promotion_bookControl extends SystemControl{
    private $links = array(
        array('url'=>'app=promotion_book&feiwa=index','text'=>'定金预售'),
        array('url'=>'app=promotion_book&feiwa=presell_goods_list', 'text'=>'全款预售'),
        array('url'=>'app=promotion_book&feiwa=book_quota_list','text'=>'套餐列表'),
        array('url'=>'app=promotion_book&feiwa=book_setting','text'=>'设置')
    );
    public function __construct(){
        ...
        if (intval($_GET['promotion_allow']) !== 1 && intval(C('promotion_allow')) !== 1){
            $url = array(
                ...
                array(
                    'url'=>'index.php?app=promotion_book&promotion_allow=1',
                    'msg'=>L('open'),
                )
            );
    ...
    public function indexFeiwa() {
        ...
        $this->book_goods_listFeiwa();
    }

    /**
     * 活动商品列表
     */
    public function book_goods_listFeiwa() {
     ...
     * 活动商品管理XML
     */
    public function get_book_goods_xmlFeiwa() {
        ...
        $order = '';
        $param = array('goods_id', 'goods_name', 'book_down_payment', 'book_final_payment', 'book_down_time', 'goods_price', 'store_id', 'store_name', 'is_own_mall');
        ...
        foreach ($goods_list as $value) {
            $param = array();
            $operation = "<a class='btn red' href='javascript:void(0);' onclick=\"fg_del('". $value['goods_id'] ."')\"><i class='fa fa-trash-o'></i>删除</a><a class='btn green' href='" . urlMall('goods', 'index', array('goods_id' => $value['goods_id'])) . "' target='_blank'><i class='fa fa-list-alt'></i>查看</a>";
            ...
            $param['store_name'] = '<a target="_blank" href="' . urlMall('show_store', 'index', array('store_id'=>$value['store_id'])) . '">' .$value['store_name'] . '</a>';
            if (isset($flippedOwnMallIds[$value['store_id']])) {
                $param['store_name'] .= '<span class="ownmall">[自营]</span>';
            }
     ...
     * 删除订金预售商品活动
     */
    public function del_book_goodsFeiwa() {
        $id = intval($_GET['id']);
        if ($id > 0) {
     ...
     * 活动商品列表
     */
    public function presell_goods_listFeiwa() {
     ...
     * 活动商品管理XML
     */
    public function get_presell_goods_xmlFeiwa() {
        $model_book = Model('p_book');
        $condition = array();
        if ($_POST['query'] != '') {
            $condition[$_POST['qtype']] = array('like', '%' . $_POST['query'] . '%');
        ...
        $order = '';
        $param = array('goods_id', 'goods_name', 'presell_deliverdate', 'goods_price', 'store_id', 'store_name', 'is_own_mall');
        if (in_array($_POST['sortname'], $param) && in_array($_POST['sortorder'], array('asc', 'desc'))) {
            $order = $_POST['sortname'] . ' ' . $_POST['sortorder'];
        }
        $page = $_POST['rp'];
     ...
     * 删除订金预售商品活动
     */
    public function del_presell_goodsFeiwa() {
        $id = intval($_GET['id']);
        if ($id > 0) {
     ...
     * 套餐列表
     */
    public function book_quota_listFeiwa() {
     ...
     * 套餐列表XML
     */
    public function get_quota_xmlFeiwa() {
        $model_book = Model('p_book');
        $condition = array();
        if ($_POST['query'] != '') {
            $condition[$_POST['qtype']] = array('like', '%' . $_POST['query'] . '%');
     ...
     * 设置
     */
    public function book_settingFeiwa() {
        // 实例化模型
        $model_setting = Model('setting');
