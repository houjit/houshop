...
class predepositControl extends SystemControl{
    ...
    public function indexFeiwa() {
        $this->predepositFeiwa();
    }

    /**
     * 充值列表
     */
    public function predepositFeiwa(){
		Tpl::setDirquna('mall');
     ...
     * 充值编辑(更改成收到款)
     */
    public function recharge_editFeiwa(){
        $id = intval($_GET['id']);
        if ($id <= 0){
            showMessage(Language::get('admin_predeposit_parameter_error'),'index.php?app=predeposit&feiwa=predeposit','','error');
        }
        //查询充值信息
        ...
        if (empty($info)){
            showMessage(Language::get('admin_predeposit_record_error'),'index.php?app=predeposit&feiwa=predeposit','','error');
        }
        if (!chksubmit()) {
            //显示支付接口列表
        ...
        try {
            ...
            showMessage(Language::get('admin_predeposit_recharge_edit_success'),'index.php?app=predeposit&feiwa=predeposit');
        } catch (Exception $e) {
            $model_pd->rollback();
            $this->log($log_msg,0);
            showMessage($e->getMessage(),'index.php?app=predeposit&feiwa=predeposit','html','error');
        }
    }
     ...
     * 充值查看
     */
    public function recharge_infoFeiwa(){
        $id = intval($_GET['id']);
        if ($id <= 0){
            showMessage(Language::get('admin_predeposit_parameter_error'),'index.php?app=predeposit&feiwa=predeposit','','error');
        }
     ...
     * 充值删除
     */
    public function recharge_delFeiwa(){
        $id = intval($_GET['id']);
        if ($id > 0) {
            $model_pd = Model('predeposit');
     ...
     * 预存款日志
     */
    public function pd_log_listFeiwa(){
		Tpl::setDirquna('mall');
     ...
     * 提现列表
     */
    public function pd_cash_listFeiwa(){
		Tpl::setDirquna('mall');
     ...
     * 删除提现记录
     */
    public function pd_cash_delFeiwa(){
        $id = intval($_GET['id']);
        if ($id > 0) {
            $model_pd = Model('predeposit');
            $condition = array();
     ...
     * 更改提现为支付状态
     */
    public function pd_cash_payFeiwa(){
        $id = intval($_GET['id']);
        if ($id <= 0){
            showMessage(Language::get('admin_predeposit_parameter_error'),'index.php?app=predeposit&feiwa=pd_cash_list','','error');
        }
        $model_pd = Model('predeposit');
        $condition = array();
        $condition['pdc_id'] = $id;
        ...
        if (!is_array($info) || count($info)<0){
            showMessage(Language::get('admin_predeposit_record_error'),'index.php?app=predeposit&feiwa=pd_cash_list','','error');
        }
        ...
        try {
            ...
            $model_pd->commit();
            $this->log($log_msg,1);
            showMessage(Language::get('admin_predeposit_cash_edit_success'),'index.php?app=predeposit&feiwa=pd_cash_list');
        } catch (Exception $e) {
            $model_pd->rollback();
            $this->log($log_msg,0);
            showMessage($e->getMessage(),'index.php?app=predeposit&feiwa=pd_cash_list','html','error');
        }
    }
     ...
     * 查看提现信息
     */
    public function pd_cash_viewFeiwa(){
        $id = intval($_GET['id']);
        $model_pd = Model('predeposit');
        $condition = array();
        $condition['pdc_id'] = $id;
    ...
    public function export_step1Feiwa(){
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){   //显示下载链接
                ...
                Tpl::output('murl','index.php?app=predeposit&feiwa=predeposit');
				Tpl::setDirquna('mall');
     ...
     *
     */
    public function export_cash_step1Feiwa(){
        $condition = array();
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){   //显示下载链接
                ...
                Tpl::output('list',$array);
                Tpl::output('murl','index.php?app=predeposit&feiwa=pd_cash_list');
				Tpl::setDirquna('mall');
                Tpl::showpage('export.excel');
            }else{  //如果数量小，直接下载
     ...
     * 预存款明细信息导出
     */
    public function export_mx_step1Feiwa(){
        $condition = array();
        if ($_GET['id'] != '') {
            $id_array = explode(',', $_GET['id']);
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){   //显示下载链接
                ...
                Tpl::output('list',$array);
                Tpl::output('murl','index.php?app=predeposit&feiwa=pd_log_list');
				Tpl::setDirquna('mall');
                Tpl::showpage('export.excel');
            }else{  //如果数量小，直接下载
    ...
    public function get_xmlFeiwa() {
        ...
        foreach ($recharge_list as $value) {
            ...
            $operation .= "<a class='btn green' href='javascript:void(0)' onclick=\"ajax_form('recharge_info','查看充值编号“". $value['pdr_sn'] ."”的明细','index.php?app=predeposit&feiwa=recharge_info&id=".$value['pdr_id']."', '640')\"><i class='fa fa-list-alt'></i>查看</a>";
     ...
     * 输出提现XML数据
     */
    public function get_cash_xmlFeiwa() {
        $model_pd = Model('predeposit');
        $condition = array();
        $if_start_date = preg_match('/^20\d{2}-\d{2}-\d{2}$/',$_GET['stime']);
        $if_end_date = preg_match('/^20\d{2}-\d{2}-\d{2}$/',$_GET['etime']);
        ...
        foreach ($cash_list as $value) {
            ...
            $param['operation'] .= "<a class='btn green' href='javascript:void(0)' onclick=\"ajax_form('cash_info','查看提现编号“". $value['pdc_sn'] ."”的明细', 'index.php?app=predeposit&feiwa=pd_cash_view&id=". $value['pdc_id'] ."', 640)\" ><i class='fa fa-list-alt'></i>查看</a>";
     ...
     * 输出预存款明细XML数据
     */
    public function get_log_xmlFeiwa() {
        $model_pd = Model('predeposit');
        $condition = array();
        if ($_POST['query'] != '') {
            $condition[$_POST['qtype']] = array('like', '%' . $_POST['query'] . '%');
