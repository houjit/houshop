...
class typeControl extends SystemControl {
    ...
    public function get_xmlFeiwa() {
        ...
        foreach ((array)$type_list as $value) {
            $param = array();
            $operation = "<a class='btn red' href='javascript:void(0);' onclick='fg_del(". $value['type_id'] .")'><i class='fa fa-trash-o'></i>删除</a><a class='btn blue' href='index.php?app=type&feiwa=type_edit&t_id=".$value['type_id']."'><i class='fa fa-pencil-square-o'></i>编辑</a>";
    ...
    public function type_addFeiwa(){
        ...
        if (chksubmit()){
            ...
            $url = array(
                array(
                    'url'=>'index.php?app=type&feiwa=type_add',
                    'msg'=>$lang['type_index_continue_to_dd']
                ),
                array(
                    'url'=>'index.php?app=type&feiwa=type',
                    'msg'=>$lang['type_index_return_type_list']
                )
            );
            $this->log(L('feiwa_add,type_index_type_name').'['.$_POST['t_mane'].']',1);
        ...
        if(is_array($spec_list) && !empty($spec_list)){
            foreach($spec_list as $k=>$val){
                ...
                $s_list[$val['class_id']]['name'] = $val['class_name']==''?L('feiwa_default'):$val['class_name'];
            }
        }
        ksort($s_list);
        // 一级分类列表
     ...
     * 编辑类型
     */
    public function type_editFeiwa(){
        $lang   = Language::getLangContent();
        if(empty($_GET['t_id'])){
        ...
        if (chksubmit()){
            ...
            if ($return){
                ...
                $this->log(L('feiwa_edit,type_index_type_name').'['.$_POST['t_mane'].']',1);
                showMessage($lang['sys_common_save_succ'],$url);
            } else {
                $this->log(L('feiwa_edit,type_index_type_name').'['.$_POST['t_mane'].']',0);
                showMessage($lang['sys_common_save_fail']);
            }
        }
     ...
     * 编辑属性
     */
    public function attr_editFeiwa(){
        $lang       = Language::getLangContent();
    ...
    public function type_delFeiwa(){
        ...
        if(is_array($type_list) && !empty($type_list)){
            ...
            $this->log(L('feiwa_delete,type_index_type_name').'[ID:'.$id.']',1);
            exit(json_encode(array('state'=>true,'msg'=>L('type_index_del_succ'))));
        }else{
            $this->log(L('feiwa_delete,type_index_type_name').'[ID:'.$id.']',0);
            exit(json_encode(array('state'=>false,'msg'=>L('param_error'))));
        }
    }
}
