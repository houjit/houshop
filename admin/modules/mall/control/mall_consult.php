...
class mall_consultControl extends SystemControl{
    ...
    public function get_xmlFeiwa(){
        ...
        foreach ($consult_list as $consult_info) {
            ...
            if ($consult_info['is_reply']) {
                $list['operation'] .= "<a class=\"btn green\" href=\"index.php?app=mall_consult&feiwa=consult_reply&id={$consult_info['mc_id']}\"><i class=\"fa fa-list-alt\"></i>编辑</a>";
            } else {
                $list['operation'] .= "<a class=\"btn green\" href=\"index.php?app=mall_consult&feiwa=consult_reply&id={$consult_info['mc_id']}\"><i class=\"fa fa-list-alt\"></i>回复</a>";
            }
     ...
     * 回复咨询
     */
    public function consult_replyFeiwa() {
        $model_mallconsult = Model('mall_consult');
        if (chksubmit()) {
            ...
            if ($result) {
                ...
                $param['param'] = array(
                    'consult_url' => urlMall('member_mallconsult', 'mallconsult_info', array('id' => $mc_id))
                );
                QueueClient::push('sendMemberMsg', $param);

                showMessage('回复成功', urlAdminMall('mall_consult', 'index'));
            } else {
                showMessage('回复失败');
            }
        }
     ...
     * 删除平台客服咨询
     */
    public function del_consultFeiwa(){
     ...
     * 批量删除平台客服咨询
     */
    public function del_consult_batchFeiwa(){
        $ids = $_POST['id'];
        if(empty($ids)){
            showMessage(Language::get('feiwa_common_del_fail'));
        }
    ...
    public function type_addFeiwa() {
        if (chksubmit()) {
            ...
            if ($result){
                $this->log('新增咨询类型',1);
                showMessage(L('feiwa_common_save_succ'), urlAdminMall('mall_consult', 'type_list'));
            }else {
                $this->log('新增咨询类型',0);
    ...
    public function type_editFeiwa() {
        ...
        if (chksubmit()) {
            ...
            if ($result){
                ...
                showMessage(L('feiwa_common_op_succ'), urlAdminMall('mall_consult', 'type_list'));
            }else {
