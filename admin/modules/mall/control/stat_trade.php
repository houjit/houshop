...
class stat_tradeControl extends SystemControl{
    private $links = array(
        array('url'=>'app=stat_trade&feiwa=income','lang'=>'stat_sale_income'),
        array('url'=>'app=stat_trade&feiwa=predeposit','lang'=>'stat_predeposit'),
        array('url'=>'app=stat_trade&feiwa=sale','lang'=>'stat_sale')
    );
    ...
    public function __construct(){
        ...
        //处理搜索时间
        if (in_array($_REQUEST['feiwa'],array('sale_trend','get_sale_xml','get_plat_sale'))){
     ...
     * 输出平台总数据
     */
    public function get_plat_incomeFeiwa(){
        $model = Model('stat');
        $where = array();
        if ($_GET['store_name']) {
            $where['ob_store_name'] = array('like',"%{$_GET['store_name']}%");
     ...
     * 输出销售收入统计XML数据
     */
    public function get_income_xmlFeiwa(){
        $model = Model('stat');
        $where = array();
        if ($_GET['store_name']) {
            $where['ob_store_name'] = array('like',"%{$_GET['store_name']}%");
        ...
        if (!empty($list) && is_array($list)){
            ...
            foreach ($list as $k => $v){
                $out_array = array();
                $out_array['operation'] = '';
                //$out_array['operation'] = '<a class="btn green" href="index.php?app=stat_trade&feiwa=sale&search_type=month&searchmonth_month='.$_GET['search_month'].
                //'&searchmonth_year='.$_GET['search_year'].'&store_name='.$v['ob_store_name'].'&order_type=40"><i class="fa fa-area-chart"></i>详细</a>';
                $out_array['ob_store_name'] = '<a href="'.urlMall('show_store', 'index', array('store_id' => $v['ob_store_id'])).'" target="_blank">'.$v['ob_store_name'].'</a>';
     ...
     * 预存款统计
     */
    public function get_predeposit_highchartsFeiwa(){
        $where = array();
        if(trim($_GET['pd_type'])=='cash_pay'){
            $field = 'sum(lg_freeze_amount) as allnum';
        }else{
     ...
     * 获取平台总预存款统计
     */
    public function get_plat_predepositFeiwa(){
        $model = Model('stat');
        $where = array();
        if(trim($_GET['pd_type'])=='cash_pay'){
            $field = 'sum(lg_freeze_amount) as allnum';
     ...
     * 输出销售收入统计XML数据
     */
    public function get_predeposit_xmlFeiwa(){
        $model = Model('stat');
        $where = array();
        if(trim($_GET['pd_type'])=='cash_pay'){
            $field = 'sum(lg_freeze_amount) as allnum';
     ...
     * 输出平台订单总数据
     */
    public function get_plat_saleFeiwa(){
        $model = Model('stat');
     ...
     * 输出订单统计XML数据
     */
    public function get_sale_xmlFeiwa(){
        $model = Model('stat');
     ...
     * 订单走势
     */
    public function sale_trendFeiwa(){
        $model = Model('stat');
        //默认统计当前数据
        if(!$this->search_arr['search_type']){
            $this->search_arr['search_type'] = 'day';
