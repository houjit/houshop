...
class vr_groupbuyControl extends SystemControl
{
    public function __construct()
    {
        ...
        if (C('groupbuy_allow') != 1) {
            showMessage('虚拟团购功能尚未开启', 'index.php?app=setting', 'html', 'error');
        }
    }
    ...
    public function class_addFeiwa()
    {
        if (chksubmit()) { //添加虚拟团购分类
            ...
            if ($res) {
                ...
                $url = array(
                    array(
                        'url'=>'index.php?app=vr_groupbuy&feiwa=class_add&parent_class_id='.$params['parent_class_id'],
                        'msg'=>'继续添加',
                    ),
                    array(
                        'url'=>'index.php?app=vr_groupbuy&feiwa=class_list',
                        'msg'=>'返回列表',
                    )
                );
                showMessage('添加成功', $url);
            } else {
                showMessage('添加失败', 'index.php?app=vr_groupbuy&feiwa=class_list', '', 'error');
            }
        }
    ...
    public function class_editFeiwa()
    {
        if (chksubmit()) {
            ...
            if ($res) {
                ...
                showMessage('编辑成功', 'index.php?app=vr_groupbuy&feiwa=class_list', '', 'succ');
            } else {
                showMessage('编辑失败', 'index.php?app=vr_groupbuy&feiwa=class_list', '', 'error');
            }
        }
    ...
    public function class_delFeiwa()
    {
        ...
        showMessage('删除成功', 'index.php?app=vr_groupbuy&feiwa=class_list', '', 'succ');
    }
     ...
     * 区域列表XML
     */
    public function area_list_xmlFeiwa()
    {
        $condition = array();
...
<span class="btn">
  ...
  <ul>
    <li><a href="index.php?app=vr_groupbuy&feiwa=area_edit&area_id={$val['area_id']}">编辑区域</a></li>
    <li><a  href="index.php?app=vr_groupbuy&feiwa=area_view&parent_area_id={$val['area_id']}">下级区域</a></li>
  </ul>
</span>
EOB;
    ...
    public function area_addFeiwa()
    {
        if (isset($_POST) && !empty($_POST)) {
            ...
            if ($res) {
                ...
                showMessage('添加成功','index.php?app=vr_groupbuy&feiwa=area_list','','succ');
            } else {
                showMessage('添加失败','index.php?app=vr_groupbuy&feiwa=area_list','','error');
            }
        }
     ...
     * 编辑区域
     */
    public function area_editFeiwa()
    {
        if (isset($_POST) && !empty($_POST)) {
            //数据验证
            $obj_validate = new Validate();
            ...
            if ($res) {
                ...
                showMessage('编辑成功', 'index.php?app=vr_groupbuy&feiwa=area_list', '', 'succ');
            } else {
                showMessage('编辑失败', 'index.php?app=vr_groupbuy&feiwa=area_list', '', 'error');
            }
        }
     ...
     * 查看区域
     */
    public function area_viewFeiwa()
    {
        //获取区域信息
        $model = Model();
     ...
     * 查看商区
     */
    public function area_streetFeiwa()
    {
        //获取区域信息
        $model = Model();
     ...
     * 删除区域
     */
    public function area_dropFeiwa()
    {
        $model = Model();
        ...
        if ($res) {
            ...
            showMessage('删除成功','index.php?app=vr_groupbuy&feiwa=area_list','','succ');
        } else {
            showMessage('删除失败','index.php?app=vr_groupbuy&feiwa=area_list','','error');
        }
    }
