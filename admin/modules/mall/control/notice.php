...
class noticeControl extends SystemControl{
    ...
    public function indexOp(){
        ...
        $member_grade_temp = $list_setting['member_grade'] = $list_setting['member_grade']?unserialize($list_setting['member_grade']):array();
        
		//提交
		if (chksubmit()){
			$content = trim($_POST['content1']);//信息内容
			$send_type = intval($_POST['send_type']);
            $member_grade = intval($_POST['member_grade']);
			...
			if ($error != ''){
				...
				if ($send_type == 2){
                    ...
                    if($member_grade>=0){
                        $insert_arr['member_id'] = 'level';
                        $insert_arr['is_level'] = $member_grade;
                    }
				} else {
				...
				showMessage(Language::get('notice_index_send_succ'),'index.php?app=notice');
        ...
        Tpl::setDirquna('mall');
        Tpl::showpage('pnotice.add');
//		Tpl::showpage('notice.add');
	}
	public function notice_tpl_editFeiwa(){
		//提交
		if (chksubmit()){
			$content = trim($_POST['content1']);//信息内容
			$send_type = intval($_POST['send_type']);
