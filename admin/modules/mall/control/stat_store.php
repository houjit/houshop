...
class stat_storeControl extends SystemControl{
    private $links = array(
        array('url'=>'app=stat_store&feiwa=newstore','lang'=>'stat_newstore'),
        array('url'=>'app=stat_store&feiwa=hotrank','lang'=>'stat_storehotrank'),
        array('url'=>'app=stat_store&feiwa=storesales','lang'=>'stat_storesales'),
        array('url'=>'app=stat_store&feiwa=degree','lang'=>'stat_storedegree'),
        array('url'=>'app=stat_store&feiwa=storearea','lang'=>'stat_storearea'),
    );
    ...
    public function __construct(){
        ...
        //处理搜索时间
        if (in_array($_REQUEST['feiwa'],array('hotrank','storesales','get_storesales_xml','storesales_list','storesales_trends'))){
     ...
     * 新增店铺
     */
    public function newstoreFeiwa(){
        //导出excel连接地址
        $actionurl = 'index.php?app=stat_store&feiwa=newstore';
        $where = array();
        //所属店铺分类
     ...
     * 热卖排行列表
     */
    public function hotrank_listFeiwa(){
        $datanum = 30;
        $model = Model('stat');
        switch ($_GET['type']){
           case 'ordernum':
     ...
     * 查看店铺列表
     */
    public function showstoreFeiwa(){
        $model = Model('stat');
        $where = array();
        if (!empty($_GET['t'])){
        ...
        $this->links[] = array('url'=>'app=stat_store&feiwa=showstore','lang'=>'stat_storelist');
     ...
     * 输出店铺列表XML数据
     */
    public function get_store_xmlFeiwa(){
        $model = Model('stat');
     ...
     * 销售统计
     */
    public function storesalesFeiwa(){
        if(!$this->search_arr['search_type']){
            $this->search_arr['search_type'] = 'day';
        }
        $model = Model('stat');
     ...
     * 店铺销售统计列表
     */
    public function storesales_listFeiwa(){
        $model = Model('stat');
        //获得搜索的开始时间和结束时间
        $searchtime_arr = $model->getStarttimeAndEndtime($this->search_arr);
        $where = array();
     ...
     * 输出店铺销售统计XML数据
     */
    public function get_storesales_xmlFeiwa(){
        $model = Model('stat');
        //获得搜索的开始时间和结束时间
        $searchtime_arr = $model->getStarttimeAndEndtime($this->search_arr);
        $where = array();
        ...
        if (!empty($list) && is_array($list)){
            foreach ($list as $k => $v){
                ...
                $out_array['operation'] = "<a class='btn green' href='javascript:void(0);' feiwa_type='showtrends' data-param='{\"storeid\":\"{$v['store_id']}\"}'><i class='fa fa-line-chart'></i>走势图</a>";
     ...
     * 销售走势
     */
    public function storesales_trendsFeiwa(){
     ...
     * 地区分布
     */
    public function storeareaFeiwa(){
        $model = Model('stat');
        //获得搜索的开始时间和结束时间
        $searchtime_arr = $model->getStarttimeAndEndtime($this->search_arr);
        $where = array();
        ...
        Tpl::output('stat_json',$stat_json);
        Tpl::output('statlist',$statlist);
        $actionurl = 'index.php?app=stat_store&feiwa=showstore&type=storearea';
        if (trim($_GET['search_time'])){
