...
class stat_industryControl extends SystemControl{
    private $links = array(
        array('url'=>'app=stat_industry&feiwa=scale','lang'=>'stat_industryscale'),
        array('url'=>'app=stat_industry&feiwa=rank','lang'=>'stat_industryrank'),
        array('url'=>'app=stat_industry&feiwa=price','lang'=>'stat_industryprice'),
        array('url'=>'app=stat_industry&feiwa=general','lang'=>'stat_industrygeneral'),
    );
    ...
    public function __construct(){
        ...
        //处理搜索时间
        if (!isset($this->search_arr['feiwa']) || in_array($this->search_arr['feiwa'],array('scale','rank','price'))){
        ...
        if (!isset($this->search_arr['feiwa']) || in_array($this->search_arr['feiwa'],array('scale','general'))){//仅显示前两级分类
            $show_depth = 2;
        }
     ...
     * 行业规模列表
     */
    public function scale_listFeiwa(){
        //获得子分类ID
     ...
     * 概况总览
     */
    public function general_listFeiwa(){
        //获得子分类ID
        $gc_childid = $gc_childarr = array();
        if ($this->choose_gcid > 0){//如果进行了分类搜索，则统计该分类下的子分类
            $gc_childdepth = $this->gc_arr[$this->choose_gcid]['depth'] + 1;
     ...
     * 输出概况总览XML数据
     */
    public function get_general_xmlFeiwa(){
        //获得子分类ID
        $gc_childid = $gc_childarr = array();
        if ($this->choose_gcid > 0){//如果进行了分类搜索，则统计该分类下的子分类
            $gc_childdepth = $this->gc_arr[$this->choose_gcid]['depth'] + 1;
