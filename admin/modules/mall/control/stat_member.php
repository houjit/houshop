...
class stat_memberControl extends SystemControl{
    private $links = array(
        array('url'=>'app=stat_member&feiwa=newmember','lang'=>'stat_newmember'),
        array('url'=>'app=stat_member&feiwa=analyze','lang'=>'stat_memberanalyze'),
        array('url'=>'app=stat_member&feiwa=scale','lang'=>'stat_scaleanalyze'),
        array('url'=>'app=stat_member&feiwa=area','lang'=>'stat_areaanalyze'),
        array('url'=>'app=stat_member&feiwa=buying','lang'=>'stat_buying'),
    );
    ...
    public function __construct(){
        ...
        //处理搜索时间
        if (!isset($this->search_arr['feiwa']) || in_array($_REQUEST['feiwa'],array('newmember','analyze','scale','area','buying','get_scale_xml'))){
     ...
     * 新增会员
     */
    public function newmemberFeiwa(){
        ...
        if($this->search_arr['search_type'] == 'day'){
            ...
            Tpl::output('actionurl','index.php?app=stat_member&feiwa=newmember&search_type=day&search_time='.date('Y-m-d',$this->search_arr['day']['search_time']));
        }
        ...
        if($this->search_arr['search_type'] == 'week'){
            ...
            Tpl::output('actionurl','index.php?app=stat_member&feiwa=newmember&search_type=week&searchweek_year='.$this->search_arr['week']['current_year'].'&searchweek_month='.$this->search_arr['week']['current_month'].'&searchweek_week='.$this->search_arr['week']['current_week']);
        }
        ...
        if($this->search_arr['search_type'] == 'month'){
            ...
            Tpl::output('actionurl','index.php?app=stat_member&feiwa=newmember&search_type=month&searchmonth_year='.$this->search_arr['month']['current_year'].'&searchmonth_month='.$this->search_arr['month']['current_month']);
        }
     ...
     * 会员分析异步详细列表
     */
    public function analyzeinfoFeiwa(){
        $model = Model('stat');
        $where = array();
     ...
     * 输出会员分析详细列表XML数据
     */
    public function get_analyzeinfo_xmlFeiwa(){
        $model = Model('stat');
        $where = array();
        $searchtime_arr_tmp = explode('|',$this->search_arr['t']);
        foreach ((array)$searchtime_arr_tmp as $k => $v){
     ...
     * 查看会员列表
     */
    public function showmemberFeiwa(){
        ...
        if ($this->search_arr['exporttype'] == 'excel'){
            ...
            //data
            foreach ($member_list as $k => $v){
                $excel_data[$k+1][] = array('data'=>$v['member_name'].'('.L('member_index_true_name,feiwa_colon').$v['member_truename'].')');
                ...
                $excel_data[$k+1][] = array('data'=>L('member_index_available,feiwa_colon').$v['available_predeposit'].L('currency_zh').'('.L('member_index_frozen,feiwa_colon').$v['freeze_predeposit'].L('currency_zh').')');
            }
            $excel_data = $excel_obj->charset($excel_data,CHARSET);
            $excel_obj->addArray($excel_data);
        ...
        $this->links[] = array('url'=>'app=stat_member&feiwa=showmember','lang'=>'stat_memberlist');
     ...
     * 输出订单统计XML数据
     */
    public function get_member_xmlFeiwa(){
        $model = Model('stat');
        $where = array();
        if (!empty($_GET['t'])){
            $searchtime_arr_tmp = explode('|',$_GET['t']);
     ...
     * 输出会员规模XML数据
     */
    public function get_scale_xmlFeiwa(){
        if(!$this->search_arr['search_type']){
            $this->search_arr['search_type'] = 'day';
        }
        $model = Model('stat');
     ...
     * 输出区域分析详细XML数据
     */
    public function get_arealist_xmlFeiwa(){
        $model = Model('stat');
        $where = array();
        $where['order_isvalid'] = 1;//计入统计的有效订单
        $searchtime_arr_tmp = explode('|',$this->search_arr['t']);
