...
class voucherControl extends SystemControl{
    ...
    public function __construct(){
        ...
        if (C('voucher_allow') != 1 || C('points_isuse')!=1){
            showMessage(Language::get('admin_voucher_unavailable'),'index.php?app=operation&feiwa=point','html','succ',1,4000);
        }
    ...
    public function settingFeiwa(){
        ...
        if (chksubmit()){
            ...
            if ($result){
                $this->log(L('admin_voucher_setting,feiwa_voucher_price_manage'));
     ...
     * 代金券面额列表
     */
    public function pricelistFeiwa()
    {
     ...
     * 代金券面额列表XML
     */
    public function pricelist_xmlFeiwa()
    {
        $model = Model();
        ...
        foreach ($list as $val) {
            $o = '<a class="confirm-del-on-click btn red" href="' . urlAdminMall('voucher', 'pricedrop', array(
                'voucher_price_id' => $val['voucher_price_id'],
            )) . '"><i class="fa fa-trash"></i>删除</a>';
            $o .= '<a class="btn green" href="' . urlAdminMall('voucher', 'priceedit', array(
    ...
    public function priceaddFeiwa(){
        if (chksubmit()){
            ...
            if ($error != ''){
                ...
                if ($rs){
                    $this->log(L('feiwa_add,admin_voucher_priceadd').'['.$_POST['voucher_price'].']');
                    showMessage(Language::get('feiwa_common_save_succ'),'index.php?app=voucher&feiwa=pricelist');
                }else {
                    showMessage(Language::get('feiwa_common_save_fail'),'index.php?app=voucher&feiwa=priceadd');
                }
            }
        }else {
     ...
     * 编辑代金券面额
     */
    public function priceeditFeiwa(){
        $id = intval($_GET['priceid']);
        if ($id <= 0){
            $id = intval($_POST['priceid']);
        }
        if ($id <= 0){
            showMessage(Language::get('param_error'),'index.php?app=voucher&feiwa=pricelist');
        }
        $model = Model();
        if (chksubmit()){
            $obj_validate = new Validate();
            ...
            if ($error != ''){
                ...
                if ($rs){
                    $this->log(L('feiwa_edit,admin_voucher_priceadd').'['.$_POST['voucher_price'].']');
                    showMessage(Language::get('feiwa_common_save_succ'),'index.php?app=voucher&feiwa=pricelist');
                }else {
                    showMessage(Language::get('feiwa_common_save_fail'),'index.php?app=voucher&feiwa=pricelist');
                }
            }
        }else {
     ...
     * 删除代金券面额
     */
    public function pricedropFeiwa(){
        ...
        if ($rs){
            $this->log(L('feiwa_del,admin_voucher_priceadd').'[ID:'.$voucher_price_id.']');
            showMessage(Language::get('feiwa_common_del_succ'),'index.php?app=voucher&feiwa=pricelist');
        }else{
            showMessage(Language::get('feiwa_common_del_fail'),'index.php?app=voucher&feiwa=pricelist');
        }
    }
     ...
     * 套餐管理
     */
    public function quotalistFeiwa(){
        $model = Model();
        //更新过期套餐的状态
        $time = time();
     ...
     * 套餐管理XML
     */
    public function quotalist_xmlFeiwa()
    {
        $condition = array();
        ...
        foreach ($list as $val) {
            ...
            $i['quota_storename'] = '<a target="_blank" href="' . urlMall('show_store', 'index', array(
     ...
     * 代金券列表
     */
    public function templatelistFeiwa()
    {
     ...
     * 代金券列表XML
     */
    public function templatelist_xmlFeiwa()
    {
        $condition = array();
        ...
        foreach ($list as $val) {
            $o = '<a class="btn blue" href="' . urlAdminMall('voucher', 'templateedit', array(
            ...
            $i['voucher_t_storename'] = '<a target="_blank" href="' . urlMall('show_store', 'index', array(
                'store_id' => $val['voucher_t_store_id'],
            )) . '">' . $val['voucher_t_storename'] . '</a>';

            if (isset($flippedOwnMallIds[$val['voucher_t_store_id']])) {
                $i['voucher_t_storename'] .= '<span class="ownmall">[自营]</span>';
            }
            //代金券店铺分类
     ...
     * 代金券模版编辑
     */
    public function templateeditFeiwa(){
        $t_id = intval($_GET['tid']);
        if ($t_id <= 0){
            $t_id = intval($_POST['tid']);
        }
        if ($t_id <= 0){
            showMessage(Language::get('param_error'),'index.php?app=voucher&feiwa=templatelist','','error');
        }
        $model = Model('voucher');
        //查询模板信息
        $param = array();
        ...
        if (empty($t_info)){
            showMessage(Language::get('param_error'),'index.php?app=voucher&feiwa=templatelist','html','error');
        }
        if(chksubmit()){
            ...
            if($rs){
                $this->log(L('feiwa_edit,feiwa_voucher_price_manage,admin_voucher_styletemplate').'[ID:'.$t_id.']');
                showMessage(Language::get('feiwa_common_save_succ'),'index.php?app=voucher&feiwa=templatelist','succ');
            }else{
                showMessage(Language::get('feiwa_common_save_fail'),'index.php?app=voucher&feiwa=templatelist','error');
            }
        }else{            
    ...
    private function show_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            case 'voucher':
                $menu_array = array(
                3=>array('menu_key'=>'templatelist','menu_name'=>Language::get('admin_voucher_template_manage'), 'menu_url'=>'index.php?app=voucher&feiwa=templatelist'),
                2=>array('menu_key'=>'quotalist','menu_name'=>Language::get('admin_voucher_quota_manage'), 'menu_url'=>'index.php?app=voucher&feiwa=quotalist'),
                5=>array('menu_key'=>'pricelist','menu_name'=>Language::get('admin_voucher_pricemanage'), 'menu_url'=>'index.php?app=voucher&feiwa=pricelist'),
                4=>array('menu_key'=>'setting','menu_name'=>Language::get('admin_voucher_setting'), 'menu_url'=>'index.php?app=voucher&feiwa=setting'),
                );
                break;
            case 'priceedit':
                $menu_array = array(
                1=>array('menu_key'=>'setting','menu_name'=>Language::get('admin_voucher_setting'), 'menu_url'=>'index.php?app=voucher&feiwa=setting'),
                2=>array('menu_key'=>'pricelist','menu_name'=>Language::get('admin_voucher_pricemanage'), 'menu_url'=>'index.php?app=voucher&feiwa=pricelist'),
                3=>array('menu_key'=>'priceedit','menu_name'=>Language::get('admin_voucher_priceedit'), 'menu_url'=>'')
                );
                break;
            case 'templateedit':
                $menu_array = array(
                1=>array('menu_key'=>'templatelist','menu_name'=>Language::get('admin_voucher_template_manage'), 'menu_url'=>'index.php?app=voucher&feiwa=templatelist'),
