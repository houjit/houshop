...
$(function(){
    $("#flexigrid").flexigrid({
        url: 'index.php?app=area&feiwa=get_xml',
        colModel : [
...
function fg_operate(name, grid) {
    if (name == 'add') {
        window.location.href = 'index.php?app=area&feiwa=add&parent_id='+cur_parent_id;
    }else if (name == 'delete') {
...
function fg_delete(id) {
	...
	$.ajax({
        type: "GET",
        dataType: "json",
        url: "index.php?app=area&feiwa=del",
        data: "area_id="+id,
...
function fg_show_children(area_id,parent_id) {
	$("#flexigrid").flexOptions({url: 'index.php?app=area&feiwa=get_xml&parent_id='+area_id}).flexReload();
...
function fg_up() {
	...
	$("#flexigrid").flexOptions({url: 'index.php?app=area&feiwa=get_xml&parent_id='+his_parent_ids[his_parent_ids.length-1]}).flexReload();
