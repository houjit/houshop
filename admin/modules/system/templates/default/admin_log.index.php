...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title">
      <div class="subject">
        <h3><?php echo $lang['sys_admin_log'];?></h3>
        <h5><?php echo $lang['sys_admin_log_subhead'];?></h5>
      </div>
    </div>
  </div>
  <!-- 操作说明 -->
...
$(function(){
    ...
    $('#ncsubmit').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?app=admin_log&feiwa=get_xml&'+$("#formSearch").serialize(),query:'',qtype:''}).flexReload();
    });
    // 高级搜索重置
    $('#ncreset').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?app=admin_log&feiwa=get_xml'}).flexReload();
        $("#formSearch")[0].reset();
    });
    $("#flexigrid").flexigrid({
        url: 'index.php?app=admin_log&feiwa=get_xml',
        colModel : [
    ...
    if (name == 'csv') {
    	...
    	if(confirm('删除后将不能恢复，确认删除吗？')){
    		$.ajax({
    	        type: "GET",
    	        dataType: "json",
    	        url: "index.php?app=admin_log&feiwa=list_del",
    	        data: "type=ago",
