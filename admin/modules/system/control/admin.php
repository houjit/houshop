...
class adminControl extends SystemControl{
    private $links = array(
        array('url'=>'app=admin&feiwa=admin','lang'=>'limit_admin'),
        array('url'=>'app=admin&feiwa=gadmin','lang'=>'limit_gadmin'),
    );
     ...
     * 管理员添加
     */
    public function admin_addFeiwa(){
        if (chksubmit()){
            $limit_str = '';
            ...
            if ($rs){
                $this->log(L('feiwa_add,limit_admin').'['.$_POST['admin_name'].']',1);
                showMessage(L('feiwa_common_save_succ'),'index.php?app=admin&feiwa=admin');
            }else {
                showMessage(L('feiwa_common_save_fail'));
            }
        }
     ...
     * 设置管理员权限
     */
    public function admin_editFeiwa(){
        if (chksubmit()){
            //没有更改密码
            if ($_POST['new_pw'] != ''){
            ...
            if ($result){
                $this->log(L('feiwa_edit,limit_admin').'[ID:'.intval($_GET['admin_id']).']',1);
                showMessage(Language::get('admin_edit_success'),'index.php?app=admin&feiwa=admin');
            }else{
                showMessage(Language::get('admin_edit_fail'),'index.php?app=admin&feiwa=admin');
            }
        }else{
            //查询用户信息
            $admin_model = Model('admin');
            ...
            if (!is_array($admininfo) || count($admininfo)<=0){
                showMessage(Language::get('admin_edit_admin_error'),'index.php?app=admin&feiwa=admin');
            }
    ...
    public function gadminFeiwa(){
        ...
        if (chksubmit()){
            ...
            if (!empty($_POST['del_id'])){
                ...
                $this->log(L('feiwa_delete,limit_gadmin').'[ID:'.implode(',',$_POST['del_id']).']',1);
                showMessage(L('feiwa_common_del_succ'));
            }else {
                showMessage(L('feiwa_common_del_fail'));
            }
     ...
     * 添加权限组
     */
    public function gadmin_addFeiwa(){
        if (chksubmit()){
            $model = Model('gadmin');
            $data['limits'] = encrypt(serialize($_POST['permission']),MD5_KEY.md5($_POST['gname']));
            $data['gname'] = $_POST['gname'];
            if ($model->insert($data)){
                $this->log(L('feiwa_add,limit_gadmin').'['.$_POST['gname'].']',1);
                showMessage(L('feiwa_common_save_succ'),'index.php?app=admin&feiwa=gadmin');
            }else {
                showMessage(L('feiwa_common_save_fail'));
            }
        }
     ...
     * 设置权限组权限
     */
    public function gadmin_editFeiwa(){
        $model = Model('gadmin');
        $gid = intval($_GET['gid']);
        ...
        if (chksubmit()){
            ...
            if ($update){
                $this->log(L('feiwa_edit,limit_gadmin').'['.$_POST['gname'].']',1);
                showMessage(L('feiwa_common_save_succ'),'index.php?app=admin&feiwa=gadmin');
            }else {
     ...
     * 组删除
     */
    public function gadmin_delFeiwa(){
        if (is_numeric($_GET['gid'])){
            Model('gadmin')->where(array('gid'=>intval($_GET['gid'])))->delete();
            $this->log(L('feiwa_delete,limit_gadmin').'[ID'.intval($_GET['gid']).']',1);
            showMessage(Language::get('feiwa_common_op_succ'),'index.php?app=admin&feiwa=gadmin');
        }else {
