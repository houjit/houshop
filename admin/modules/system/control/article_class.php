...
class article_classControl extends SystemControl{
    ...
    public function indexFeiwa() {
        $this->article_classFeiwa();
    }

    /**
     * 文章管理
     */
    public function article_classFeiwa(){
     ...
     * 文章分类 新增
     */
    public function article_class_addFeiwa(){
        ...
        if (chksubmit()){
            ...
            if ($error != ''){
                ...
                if ($result){
                    $url = array(
                        array(
                            'url'=>'index.php?app=article_class&feiwa=article_class_add&ac_parent_id='.intval($_POST['ac_parent_id']),
                            'msg'=>$lang['article_class_add_class'],
                        ),
                        array(
                            'url'=>'index.php?app=article_class&feiwa=article_class',
                            'msg'=>$lang['article_class_add_back_to_list'],
                        )
                    );
                    $this->log(l('feiwa_add,article_class_index_class').'['.$_POST['ac_name'].']',1);
     ...
     * 文章分类编辑
     */
    public function article_class_editFeiwa(){
        $lang   = Language::getLangContent();
        $model_class = Model('article_class');
...
//              $update_array['ac_parent_id'] = intval($_POST['ac_parent_id']);
                ...
                if ($result){
                    $url = array(
                        ...
                        array(
                            'url'=>'index.php?app=article_class&feiwa=article_class_edit&ac_id='.intval($_POST['ac_id']),
                            'msg'=>$lang['article_class_edit_again'],
                        ),
                    );
                    $this->log(l('feiwa_edit,article_class_index_class').'['.$_POST['ac_name'].']',1);
                    showMessage($lang['article_class_edit_succ'],'index.php?app=article_class&feiwa=article_class');
                }else {
     ...
     * 删除分类
     */
    public function article_class_delFeiwa(){
        $lang   = Language::getLangContent();
        $model_class = Model('article_class');
        if (intval($_GET['ac_id']) > 0){
            ...
            $this->log(l('feiwa_add,article_class_index_class').'[ID:'.intval($_GET['ac_id']).']',1);
            showMessage($lang['article_class_index_del_succ'],'index.php?app=article_class&feiwa=article_class');
        }else {
            showMessage($lang['article_class_index_choose'],'index.php?app=article_class&feiwa=article_class');
        }
    }
    /**
     * ajax操作
