...
class messageControl extends SystemControl{
    private $links = array(
        array('url'=>'app=message&feiwa=email','lang'=>'email_set'),
        array('url'=>'app=message&feiwa=email_tpl','lang'=>'email_tpl')
    );
    ...
    public function emailFeiwa(){
        ...
        if (chksubmit()){
            ...
            if ($result === true){
                $this->log(L('feiwa_edit,email_set'),1);
                showMessage(L('feiwa_common_save_succ'));
            }else {
                $this->log(L('feiwa_edit,email_set'),0);
     ...
     * 邮件模板列表
     */
    public function email_tplFeiwa(){
     ...
     * 编辑邮件模板
     */
    public function email_tpl_editFeiwa(){
        $model_templates = Model('mail_templates');
        if (chksubmit()){
            ...
            if ($error != ''){
                ...
                if ($result === true){
                    $this->log(L('feiwa_edit,email_tpl'),1);
                    showMessage(L('mailtemplates_edit_succ'),'index.php?app=message&feiwa=email_tpl');
                }else {
                    $this->log(L('feiwa_edit,email_tpl'),0);
   ...
   /**
	 ...
	 * @return
	 */
	public function email_testingFeiwa(){
		/**
		 * 读取语言包
		 */
