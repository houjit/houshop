...
class articleControl extends SystemControl{
    ...
    public function get_xmlFeiwa(){
        ...
        if (is_array($article_list)){
            ...
            foreach ($article_list as $k => $v){
                $list = array();
                $list['operation'] = "<a class='btn red' onclick=\"fg_delete({$v['article_id']})\"><i class='fa fa-trash-o'></i>删除</a><a class='btn blue' href='index.php?app=article&feiwa=article_edit&article_id={$v['article_id']}'><i class='fa fa-pencil-square-o'></i>编辑</a>";
                ...
                if ($v['ac_id'] == 1) {
                    $list['ac_name'] .= ' ['.str_replace(array(ARTICLE_POSIT_MALL,ARTICLE_POSIT_BUYER,ARTICLE_POSIT_SELLER,ARTICLE_POSIT_ALL,2,3,4), array('商城前台','买家中心','商家中心','全站'), $v['article_position']).'显示]';
                }
     ...
     * 文章添加
     */
    public function article_addFeiwa(){
        $lang   = Language::getLangContent();
        $model_article = Model('article');
        /**
         * 保存
...
//                 array("input"=>$_POST["article_content"], "require"=>"true", "message"=>$lang['article_add_content_null']),
            ...
            if ($error != ''){
                ...
                if ($result){
                    //fengstaa
                    @unlink(BASE_DATA_PATH.DS.'cache'.DS.'index/article.php');
                    //
                    /**
                     * 更新图片信息ID
                     */
                    ...
                    $url = array(
                        array(
                            'url'=>'index.php?app=article&feiwa=article',
                            'msg'=>"{$lang['article_add_tolist']}",
                        ),
                        array(
                            'url'=>'index.php?app=article&feiwa=article_add&ac_id='.intval($_POST['ac_id']),
...
//                 array("input"=>$_POST["article_content"], "require"=>"true", "message"=>$lang['article_add_content_null']),
                ...
                if ($result){
                   ...
                   // @unlink(BASE_DATA_PATH.DS.'cache'.DS.'index/article.php');
                    ...
                    $url = array(
                        ...
                        array(
                            'url'=>'index.php?app=article&feiwa=article_edit&article_id='.intval($_POST['article_id']),
     ...
     * 文章图片上传
     */
    public function article_pic_uploadFeiwa(){
        /**
         * 上传图片
         */
