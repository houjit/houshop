...
class reads_picture_classControl extends SystemControl{
    ...
    public function indexFeiwa() {
        $this->reads_picture_class_listFeiwa();
    }

    /**
     * 资讯画报分类列表
     **/
    public function reads_picture_class_listFeiwa() {
        $model = Model('reads_picture_class');
        $list = $model->getList(TRUE);
        $this->show_menu('list');
        Tpl::output('list',$list);
        Tpl::setDirquna('reads');
Tpl::showpage("reads_picture_class.list");
    }

    /**
     * 资讯画报分类添加
     **/
    public function reads_picture_class_addFeiwa() {
        $this->show_menu('add');
        Tpl::setDirquna('reads');
Tpl::showpage('reads_picture_class.add');
    }

    /**
     * 资讯画报分类保存
     **/
    public function reads_picture_class_saveFeiwa() {
        ...
        $model_class = Model('reads_picture_class');
        $result = $model_class->save($param);
        if($result) {
            $this->log(Language::get('reads_log_picture_class_save').$result, 1);
            showMessage(Language::get('class_add_success'),'index.php?app=reads_picture_class&feiwa=reads_picture_class_list');
        } else {
            $this->log(Language::get('reads_log_picture_class_save').$result, 0);
            showMessage(Language::get('class_add_fail'),'index.php?app=reads_picture_class&feiwa=reads_picture_class_list','','error');
        }
     ...
     * 资讯画报分类删除
     **/
     public function reads_picture_class_dropFeiwa() {
        ...
        if($result) {
            $this->log(Language::get('reads_log_picture_class_drop').$result, 1);
            showMessage(Language::get('class_drop_success'),'');
        } else {
            $this->log(Language::get('reads_log_picture_class_drop').$result, 0);
    ...
    private function show_menu($menu_key) {
        $menu_array = array(
            'list'=>array('menu_type'=>'link','menu_name'=>Language::get('feiwa_list'),'menu_url'=>'index.php?app=reads_picture_class&feiwa=reads_picture_class_list'),
            'add'=>array('menu_type'=>'link','menu_name'=>Language::get('feiwa_new'),'menu_url'=>'index.php?app=reads_picture_class&feiwa=reads_picture_class_add'),
        );
