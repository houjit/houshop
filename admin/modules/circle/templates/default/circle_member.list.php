...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title">
      <div class="subject">
        <h3><?php echo $lang['sys_circle_membermanage'];?></h3>
        <h5><?php echo $lang['sys_circle_membermanage_subhead'];?></h5>
      </div>
    </div>
  </div>
...
$(function(){
    $("#flexigrid").flexigrid({
        url: 'index.php?app=circle_member&feiwa=get_xml',
        colModel : [
...
function fg_del(id) {
    if(confirm('删除后将不能恢复，确认删除这项吗？')){
        $.getJSON('index.php?app=circle_member&feiwa=member_del', {id:id}, function(data){
            if (data.state) {
...
function fg_recommend(id, value) {
    $.getJSON('index.php?app=circle_member&feiwa=member_recommend', {id:id, value:value}, function(data){
        if (data.state) {
            $("#flexigrid").flexReload();
        } else {
            showError(data.msg)
