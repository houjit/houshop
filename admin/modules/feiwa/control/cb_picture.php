...
class reads_pictureControl extends SystemControl{
    //文章状态草稿箱
    ...
    public function indexFeiwa() {
        $this->reads_picture_listFeiwa();
    }

    /**
     * 资讯文章列表
     **/
    public function reads_picture_listFeiwa()
    {
     ...
     * 待审核文章列表
     */
    public function reads_picture_list_verifyFeiwa()
    {
     ...
     * 已发布文章列表
     */
    public function reads_picture_list_publishedFeiwa()
    {
...
Tpl::showpage("reads_picture.list");
    }

    public function reads_picture_list_xmlFeiwa()
    {
        $condition = array();
        ...
        foreach ($list as $val) {
            ...
            $o .= '<li><a target="_blank" href="' .
                    READS_SITE_URL .
                    '/index.php?app=picture&feiwa=picture_detail&picture_id=' .
                    $val['picture_id'] .
                    '">查看画报</a></li>';
            ...
            $img = getREADSArticleImageUrl($val['picture_attachment_path'], $val['picture_image']);
            $i['img'] = <<<EOB
...
EOB;
     ...
     * 资讯文章审核
     */
    public function reads_picture_verifyFeiwa() {
        if(intval($_REQUEST['verify_state']) === 1) {
            $this->reads_picture_state_modify(self::ARTICLE_STATE_PUBLISHED);
        } else {
            $this->reads_picture_state_modify(self::ARTICLE_STATE_DRAFT, array('picture_verify_reason' => $_POST['verify_reason']));
        }
    }
     ...
     * 资讯文章回收
     */
    public function reads_picture_callbackFeiwa() {
        $this->reads_picture_state_modify(self::ARTICLE_STATE_DRAFT);
    }

    /**
     * 修改文章状态
     */
    private function reads_picture_state_modify($new_state, $param = array()) {
     ...
     * 资讯文章分类排序修改
     */
    public function update_picture_sortFeiwa() {
        if(intval($_GET['id']) <= 0) {
        ...
        if ($new_sort > 255){
            ...
            $model_class = Model("reads_picture");
     ...
     * 资讯文章分类排序修改
     */
    public function update_picture_clickFeiwa() {
     ...
     * 资讯文章删除
     **/
     public function reads_picture_dropFeiwa() {
        ...
        if($result) {
            $this->log(Language::get('reads_log_picture_drop').$picture_id, 1);
            showMessage(Language::get('feiwa_common_del_succ'),'');
        } else {
            $this->log(Language::get('reads_log_picture_drop').$picture_id, 0);
    ...
    private function show_menu($menu_key) {
        $menu_array = array(
            'list'=>array('menu_type'=>'link','menu_name'=>Language::get('feiwa_list'),'menu_url'=>'index.php?app=reads_picture&feiwa=reads_picture_list'),
            'list_verify'=>array('menu_type'=>'link','menu_name'=>Language::get('reads_article_list_verify'),'menu_url'=>'index.php?app=reads_picture&feiwa=reads_picture_list_verify'),
            'list_published'=>array('menu_type'=>'link','menu_name'=>Language::get('reads_article_list_published'),'menu_url'=>'index.php?app=reads_picture&feiwa=reads_picture_list_published'),
        );
