...
class dbControl extends SystemControl{
	...
	public function dbFeiwa(){
		...
		if ($_POST['form_submit'] == 'ok'){
			 ...
			 * 验证文件夹是否重复
			 */
			if (is_dir(BASE_DATA_PATH.DS.'feiwa_sql_back'.DS.$_POST['backup_name'])) {
			...
			$_SESSION['db_backup']['feiwa'] = 'create';
			...
			showMessage($lang['db_index_backup_to_wait'],'index.php?app=db&feiwa=db&step=1');
		}
		/**
		 * 数据库备份步骤
		 */
		if (intval($_GET['step']) >= 1){
			 ...
			 */
			if (
				$_SESSION['db_backup']['feiwa'] == 'insert' && 
				...
				$url = array(
					array(
						'url'=>'index.php?app=db&feiwa=db&step='.(intval($_GET['step'])+1),
	 ...
	 * 数据库恢复
	 */
	public function db_restoreFeiwa(){
		$lang 	= Language::getLangContent();
		/**
		 * 删除
		 */
		if ($_POST['form_submit'] == 'ok'){
			if (!empty($_POST['dir_name']) && is_array($_POST['dir_name'])){
				$dir = BASE_DATA_PATH.DS.'feiwa_sql_back';
				foreach ($_POST['dir_name'] as $k => $v){
					if (file_exists(BASE_DATA_PATH.DS.'feiwa_sql_back'.DS.$v)){
						$file_list  = array();
		...
		$tmp_list = readDirList(BASE_DATA_PATH.DS.'feiwa_sql_back');
		/**
		 * 整理内容
		 */
		$dir_list = array();
		if (is_array($tmp_list)){
			foreach ($tmp_list as $k => $v){
				$dir_list[$k]['name'] = $v;
				$dir_list[$k]['make_time'] = date('Y-m-d H:i:s',filemtime(BASE_DATA_PATH.DS.'feiwa_sql_back'.DS.$v));
				$dir_list[$k]['size'] = number_format((getDirSize(BASE_DATA_PATH.DS.'feiwa_sql_back'.DS.$v)/1024),2).'KB';
				$dir_list[$k]['file_num'] = count(glob(BASE_DATA_PATH.DS.'feiwa_sql_back'.DS.$v.DS."*.sql"));
			}
		}
	 ...
	 * 数据库备份导入
	 */
	public function db_importFeiwa(){
		$lang 	= Language::getLangContent();
		if ($_GET['dir_name'] != '' && file_exists(BASE_DATA_PATH.DS.'feiwa_sql_back'.DS.$_GET['dir_name'])){
			$model_db = Model('db');
			...
			if ($result == 'succ'){
				$url = array(
					array(
						'url'=>'index.php?app=db&feiwa=db_restore',
			...
			if ($result == 'continue'){
				$url = array(
					array(
						'url'=>'index.php?app=db&feiwa=db_import&dir_name='.$_GET['dir_name'].'&step='.(intval($_GET['step'])+1),
	 ...
	 * 数据库 删除
	 */
	public function db_delFeiwa(){
		$lang 	= Language::getLangContent();
		if ($_GET['dir_name'] != '' && file_exists(BASE_DATA_PATH.DS.'feiwa_sql_back'.DS.$_GET['dir_name'])){
			$dir = BASE_DATA_PATH.DS.'feiwa_sql_back'.DS.$_GET['dir_name'];
			$file_list = array();
			 ...
			 * 删除目录
			 */
			@rmdir(BASE_DATA_PATH.DS.'feiwa_sql_back'.DS.$_GET['dir_name']);
	...
	public function ajaxFeiwa(){
		switch ($_GET['branch']){
			 ...
			 */
			case 'db_file':
				if ($_GET['dir_name'] != '.' && $_GET['dir_name'] != '..' && is_dir(BASE_DATA_PATH.DS.'feiwa_sql_back'.DS.$_GET['dir_name'])){
					$dir = BASE_DATA_PATH.DS.'feiwa_sql_back'.DS.$_GET['dir_name'];
					$tmp = array();
