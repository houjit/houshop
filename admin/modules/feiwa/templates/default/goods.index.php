...
function fg_operation(name, bDiv) {
    if (name == 'csv') {
        ...
        fg_csv(itemids);
	        ...
	        window.location.href = '<?php echo urladminmall('evaluate','evalgoods_list');?>';
    }
}
...
function fg_invite(ids) {
    _uri = "index.php?app=goods&feiwa=goods_invite&id=" + ids;
    CUR_DIALOG = ajax_form('goods_invite', '分销设置', _uri, 640);
}
//添加评论
function fg_pinglun(gids) {
    _uri = "index.php?app=goods&feiwa=add&goods_id=" + gids;
    CUR_DIALOG = ajax_form('goods_pinlun', '为此商品添加评论', _uri, 640);
}

function fg_sku(commonid) {
    CUR_DIALOG = ajax_form('login','商品"' + commonid +'"的SKU列表','index.php?app=goods&feiwa=get_goods_sku_list&commonid=' + commonid, 580);
}
// 删除
function fg_del(id) {
