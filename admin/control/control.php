 ...
 * @
 */
defined('INSYSVIEW') or exit('Access Invalid bbbbbb!');
class SystemControl{
    ...
    protected function __construct() {
        ...
        if (($_GET['branch']!='' || $_GET['feiwa']=='ajax') && strtoupper(CHARSET) == 'GBK'){
    ...
    protected final function systemLogin() {
        ...
        if (!key_exists('gid',(array)$user) || !isset($user['sp']) || (empty($user['name']) || empty($user['id']))){
            @header('Location: '. ADMIN_SITE_URL .'/index.php?app=login&feiwa=login');exit;
        }else {
    ...
    protected final function checkPermission($link_nav = null) {
        ...
        $app = $_GET['app'] ? $_GET['app'] : $_POST['app'];
        $app = $app == '' ? 'index' : $app;
        $permission = $this->getPermission();
        
        if (!defined('MODULE_NAME')) return true;//modules目录外的不需要验证
        if (is_array($permission[MODULE_NAME]) && in_array($app, $permission[MODULE_NAME])){
            return true;
        }
        showMessage(Language::get('feiwa_assign_right'),'','html','succ',0);
    }
    ...
    protected final function sublink($links = array(), $actived = '', $file='index.php') {
        ...
        foreach ($links as $k=>$v) {
            ...
            if (empty($array['feiwa'])) $array['feiwa'] = 'index';
            if (!$this->checkPermission($array)) continue;
            $href = ($array['feiwa'] == $actived ? null : "href=\"{$file}?{$v['url']}\"");
            $class = ($array['feiwa'] == $actived ? "class=\"current\"" : null);
    ...
    protected final function log($lang = '', $state = 1, $admin_name = '', $admin_id = 0) {
        ...
        $data['ip']         = getIp();
        $data['url']        = $_REQUEST['app'].'&'.$_REQUEST['feiwa'];
