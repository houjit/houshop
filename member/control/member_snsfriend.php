...
class member_snsfriendControl extends BaseMemberControl {
     ...
     * 加关注
     */
    public function addfollowFeiwa() {
        $mid = intval($_GET['mid']);
        if($mid<=0){
     ...
     * 加关注
     */
    public function delfollowFeiwa() {
        $mid = intval($_GET['mid']);
        if($mid<=0){
            showDialog(Language::get('wrong_argument'),'','error');
        }
     ...
     * 批量加关注
     */
    public function batch_addfollowFeiwa() {
        // 验证参数
        if(trim($_GET['ids']) == ''){
            showDialog(Language::get('wrong_argument'), '', 'error');
        }
    ...
    private function profile_menu($menu_key) {
        $menu_array = array(
            1=>array('menu_key'=>'find',    'menu_name'=>Language::get('snsfriend_findmember'), 'menu_url'=>'index.php?app=member_snsfriend&feiwa=find'),
            2=>array('menu_key'=>'follow',  'menu_name'=>Language::get('snsfriend_follow'), 'menu_url'=>'index.php?app=member_snsfriend&feiwa=follow'),
            3=>array('menu_key'=>'fan',     'menu_name'=>Language::get('snsfriend_fans'),   'menu_url'=>'index.php?app=member_snsfriend&feiwa=fan')
        );
