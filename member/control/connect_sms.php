...
class connect_smsControl extends BaseLoginControl{
     ...
     * 短信动态码
     */
    public function get_captchaFeiwa(){
        $state = '发送失败';
        $phone = $_GET['phone'];
        if (strlen($phone) == 11){
            ...
            if(!empty($sms_log) && ($sms_log['add_time'] > TIMESTAMP-600)) {//同一IP十分钟内只能发一条短信
                ...
                if($state == 'true'){
                    ...
                    if(C('feiwa_sms_type')==4){
                        ...
                        if(preg_match('/12-1/', $result)) {
                            $state = '手机短信发送失败,检查是否设置签名:'.$result;
                        }elseif (preg_match('/未知错误/', $result)) {
                            $state = '手机短信发送失败'.$result;
                        }else{
                            $state = $result;
                        }
     ...
     * 验证注册动态码
     */
    public function check_captchaFeiwa(){
        $state = '验证失败';
        $phone = $_GET['phone'];
