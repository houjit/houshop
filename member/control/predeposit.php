...
class predepositControl extends BaseMemberControl {
     ...
     * 充值添加
     */
    public function recharge_addFeiwa(){
        if (!chksubmit()){
            //信息输出
        ...
        if ($insert) {
            //转向到商城支付页面
            redirect(MALL_SITE_URL . '/index.php?app=buy&feiwa=pd_pay&pay_sn='.$pay_sn);
        }
    }
     ...
     * 平台充值卡
     */
    public function rechargecard_addFeiwa()
    {
        if (!chksubmit()) {
     ...
     *
     */
    public function recharge_showFeiwa(){
    ...
    public function recharge_delFeiwa(){
        ...
        if ($result){
            showDialog(Language::get('feiwa_common_del_succ'),'reload','succ','CUR_DIALOG.close()');
        }else {
     ...
     * 充值卡余额变更日志
     */
    public function rcb_log_listFeiwa()
    {
        $model = Model();
     ...
     * 申请提现
     */
    public function pd_cash_addFeiwa(){
        if (chksubmit()){
            ...
            if (floatval($member_info['available_predeposit']) < $pdc_amount){
                showDialog(Language::get('predeposit_cash_shortprice_error'),'index.php?app=predeposit&feiwa=pd_cash_list','error');
            }
            try {
                ...
                $model_pd->commit();
                showDialog(Language::get('predeposit_cash_add_success'),'index.php?app=predeposit&feiwa=pd_cash_list','succ','CUR_DIALOG.close()');
            } catch (Exception $e) {
                $model_pd->rollback();
                showDialog($e->getMessage(),'index.php?app=predeposit&feiwa=pd_cash_list','error');
            }
        }
    }
     ...
     * 提现记录详细
     */
    public function pd_cash_infoFeiwa(){
        $pdc_id = intval($_GET["id"]);
        if ($pdc_id <= 0){
            showMessage(Language::get('predeposit_parameter_error'),'index.php?app=predeposit&feiwa=pd_cash_list','html','error');
        }
        $model_pd = Model('predeposit');
        $condition = array();
        $condition['pdc_member_id'] = $_SESSION['member_id'];
        ...
        if (empty($info)){
            showMessage(Language::get('predeposit_record_error'),'index.php?app=predeposit&feiwa=pd_cash_list','html','error');
        }
    ...
    private function profile_menu($menu_type,$menu_key=''){
        $menu_array = array(
            ...
            array('menu_key'=>'rcb_log_list',   'menu_name'=>'充值卡余额',   'menu_url'=>'index.php?app=predeposit&feiwa=rcb_log_list',),
        );
        switch ($menu_type) {
            case 'rechargeinfo':
            ...
            case 'rechargecard_add':
                ...
                break;
            case 'cashadd':
                $menu_array[] = array('menu_key'=>'cashadd','menu_name'=>'提现申请',    'menu_url'=>'index.php?app=predeposit&feiwa=pd_cash_add');
                break;
            case 'cashinfo':
