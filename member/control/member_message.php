...
class member_messageControl extends BaseMemberControl {
     ...
     * @return
     */
    public function personalmsgFeiwa() {
        $model_message  = Model('message');
        $page   = new Page();
        $page->setEachNum(10);
        $page->setStyle('admin');
     ...
     * @return
     */
    public function privatemsgFeiwa(){
        $model_message  = Model('message');
        $page   = new Page();
        $page->setEachNum(10);
        $page->setStyle('admin');
     ...
     * @return
     */
    public function systemmsgFeiwa(){
        
        $model_member = Model('member');
        $member_grade = $model_member->getMemberGradeArr();
        foreach ($member_grade as $key => $value) {
            $next_level = $member_grade[$key+1];
            if(!$next_level) $next_level['exppoints']='null';
            $newarr[$value['level']] = $value['exppoints'].'-'.$next_level['exppoints'];
        }
        
        
        ////
        $memberInfo = $model_member->getMemberInfoByID($_SESSION['member_id']);
        $member_exppoints = $memberInfo['member_exppoints'];
        $member_grades = $model_member->getOneMemberGrade($member_exppoints);
        /////
        ...
        $page->setEachNum(10);
        $page->setStyle('admin');
        $message_array	= $model_message->listMessage(array('from_member_id'=>'0','message_type'=>'1','to_member_id'=>$_SESSION['member_id'],'no_del_member_id'=>$_SESSION['member_id'],'is_level'=>$member_grades['level']),$page);
    ...
    public function savemsgFeiwa() {
        ...
        if (chksubmit()) {
            ...
            if($_GET['type'] == 'sns_board'){
                ...
                showDialog(Language::get('home_message_send_success'),'index.php?app=member_message&feiwa=privatemsg','succ');
            }
        }
    }
    /**
     ...
     * @return
     */
    public function showmsgcommonFeiwa() {
        $model_message  = Model('message');
     ...
     * @return
     */
    public function showmsgbatchFeiwa() {
        $model_message  = Model('message');
        $message_id =  intval($_GET['message_id']);
        $drop_type = trim($_GET['drop_type']);
        $referer_url = getReferer();
     ...
     * @return
     */
    public function savereplyFeiwa() {
        //查询会员是否允许发送站内信
        $isallowsend = $this->allowSendMessage($_SESSION['member_id']);
        if (!$isallowsend){
            if($_GET['inajax'] == 1){
                ...
                showMessage(Language::get('home_message_noallowsend'),'index.php?app=member_message&feiwa=message','html','error');
            }
        }
        if ($_POST['form_submit'] == 'ok') {
            $message_id = intval($_POST["message_id"]);
            if ($message_id <=0){
                showMessage(Language::get('wrong_argument'),'index.php?app=member_message&feiwa=message','html','error');
            }
            $obj_validate = new Validate();
            $obj_validate->validateparam = array(
            ...
            if($_GET['inajax'] == 1){
                ...
                showMessage(Language::get('home_message_send_success'),'index.php?app=member_message&feiwa=privatemsg');
            }
        }else {
            if($_GET['inajax'] == 1){
     ...
     * 删除普通信
     */
    public function dropcommonmsgFeiwa() {
     ...
     * 删除批量站内信
     */
    public function dropbatchmsgFeiwa() {
        $message_id = trim($_GET['message_id']);
        $drop_type = trim($_GET['drop_type']);
    ...
    public function settingFeiwa() {
        ...
        if (chksubmit()) {
            ...
            if ($result) {
                showDialog(L('feiwa_common_save_succ'), '', 'succ');
            } else {
    ...
    private function profile_menu($menu_key='') {
        $menu_array = array(
            array('menu_key'=>'message', 'menu_name'=>Language::get('home_message_received_message'),'menu_url'=>'index.php?app=member_message&feiwa=message'),
            array('menu_key'=>'private', 'menu_name'=>Language::get('home_message_private_message'), 'menu_url'=>'index.php?app=member_message&feiwa=privatemsg'),
            array('menu_key'=>'system',  'menu_name'=>Language::get('home_message_system_message'),  'menu_url'=>'index.php?app=member_message&feiwa=systemmsg'),
            array('menu_key'=>'ggmsg',  'menu_name'=>'系统公告',  'menu_url'=>'index.php?app=member_message&feiwa=ggmsg'),
            array('menu_key'=>'close',   'menu_name'=>Language::get('home_message_close'),   'menu_url'=>'index.php?app=member_message&feiwa=personalmsg'),
            array('menu_key'=>'setting', 'menu_name'=>'接收设置', 'menu_url'=>urlMember('member_message', 'setting'))
        );
        if($menu_key == 'sendmsg') {
            $menu_array[] = array('menu_key'=>'sendmsg','menu_name'=>Language::get('home_message_send_message'),'menu_url'=>'index.php?app=member_message&feiwa=sendmsg');
