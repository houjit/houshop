...
<div class="wrap">
  ...
  <div class="alert alert-success">
    ...
    <ul>
      ...
      <li>2. 如果您的邮箱已失效，可以 <a href="index.php?app=member_security&feiwa=auth&type=modify_mobile">绑定手机</a> 后通过接收手机短信完成验证。</li>
      <li>3. 如果您的手机已失效，可以 <a href="index.php?app=member_security&feiwa=auth&type=modify_email">绑定邮箱</a> 后通过接收邮件完成验证。</li>
      <li>4. 请正确输入下方图形验证码，如看不清可点击图片进行更换，输入完成后进行下一步操作。</li>
      <li>5. 收到安全验证码后，请在30分钟内完成验证。</li>
    </ul>
  </div>
  <div class="ncm-default-form">
    <form method="post" id="auth_form" action="index.php?app=member_security&feiwa=auth">
...
$(function(){
	...
	$('#send_auth_code').on('click',function(){
		...
		ALLOW_SEND = !ALLOW_SEND;
		$('#sending').show();
		$.getJSON('index.php?app=member_security&feiwa=send_auth_code',{type:$('#auth_type').val()},function(data){
			if (data.state == 'true') {
				$('#sending').hide();
				$('#show_times').html(60);
