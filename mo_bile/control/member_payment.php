...
class member_paymentControl extends mobileMemberControl {
    ...
    public function __construct() {
        ...
        if($_GET['feiwa'] != 'payment_list' && !$_POST['payment_code']) {
            $payment_code = 'alipay';

            if(in_array($_GET['feiwa'], array('wx_app_pay', 'wx_app_pay3', 'wx_app_vr_pay', 'wx_app_vr_pay3'), true)) {
                $payment_code = 'wxpay';
            }
            else if (in_array($_GET['feiwa'],array('alipay_native_pay','alipay_native_vr_pay'),true)) {
     ...
     * 虚拟订单支付 新方法
     */
    public function vr_pay_newFeiwa() {
        @header("Content-type: text/html; charset=".CHARSET);
    ...
    private function _api_pay($order_pay_info) {
        $ifeiwa_file = BASE_PATH.DS.'api'.DS.'payment'.DS.$this->payment_code.DS.$this->payment_code.'.php';
        if(!is_file($ifeiwa_file)){
            exit('支付接口不存在');
        }
        require($ifeiwa_file);
     ...
     * 可用支付参数列表
     */
    public function payment_listFeiwa() {
        $model_mb_payment = Model('mb_payment');
     ...
     * 微信APP订单支付
     */
    public function wx_app_payFeiwa() {
        $pay_sn = $_POST['pay_sn'];
     ...
     * 微信APP虚拟订单支付
     */
    public function wx_app_vr_payFeiwa() {
        $pay_sn = $_POST['pay_sn'];
     ...
     * 微信APP订单支付
     */
    public function wx_app_pay3Feiwa() {
        $pay_sn = $_POST['pay_sn'];
     ...
     * 微信APP虚拟订单支付
     */
    public function wx_app_vr_pay3Feiwa() {
        $pay_sn = $_POST['pay_sn'];
     ...
     * 取得支付宝移动支付 订单信息 实物订单
     */
    public function alipay_native_payFeiwa() {
        $pay_sn = $_POST['pay_sn'];
     ...
     * 取得支付宝移动支付 订单信息 虚拟订单
     */
    public function alipay_native_vr_payFeiwa() {
        $pay_sn = $_POST['pay_sn'];
        if (!preg_match('/^\d+$/',$pay_sn)){
            output_error('支付单号错误');
        }
