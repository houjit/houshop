...
class mobileDeliveryControl extends mobileControl{
    public function __construct() {
        parent::__construct();
        
        $model = Model();
        $key = $_POST['key']?$_POST['key']:$_GET['key'];
        $datas_mb_delivery_token['token'] = $key;
        $this->mb_delivery_token = $model->table('mb_delivery_token')->where($datas_mb_delivery_token)->find();
        $datas_delivery_info['dlyp_id'] =  $this->mb_delivery_token['dlyp_id'];
        $this->delivery_info = $model->table('delivery_point')->where($datas_delivery_info)->find();
