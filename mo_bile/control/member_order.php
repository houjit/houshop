...
class member_orderControl extends mobileMemberControl {
    ...
    public function order_listFeiwa() {
        ...
        foreach ($order_list_array as $value) {
            ...
            $value['ownmall'] = in_array($value['store_id'], $ownMallIds);
            ...
            foreach ($value['extend_order_goods'] as $k => $goods_info) {
				...
				$value['extend_order_goods'][$k]['ifpf'] = Model('goods')->getifpf($value['extend_order_goods'][$k]['goods_id'],$value['extend_order_goods'][$k]['goods_price']);
            }
     ...
     * 取消订单
     */
    public function order_deleteFeiwa() {
        $model_order = Model('order');
        $logic_order = Logic('order');
        $order_id = intval($_POST['order_id']);
     ...
     * 订单确认收货
     */
    public function order_receiveFeiwa() {
        $model_order = Model('order');
        $logic_order = Logic('order');
        $order_id = intval($_POST['order_id']);
     ...
     * 取得当前的物流最新信息
     */
    public function get_current_deliverFeiwa(){
        $order_id   = intval($_POST['order_id']);
        if ($order_id <= 0) {
            output_error('订单不存在');
        }
...
//         $data['refund_state'] = $result['data']['order_info']['refund_state'];
        ...
        foreach ($result['data']['order_info']['goods_list'] as $_k => $_v) {
			...
			$data['goods_list'][$_k]['ifpf'] = Model('goods')->getifpf($_v['goods_id'],$_v['goods_price']);
        }
        ...
        $ownMallIds = Model('store')->getOwnMallIds();
        $data['ownmall'] = in_array($data['store_id'], $ownMallIds);
