...
class goodsControl extends mobileHomeControl{
    ...
    public function goods_listFeiwa() {
        ...
        //所需字段
        $fieldstr = "goods_id,goods_commonid,store_id,goods_name,goods_jingle,goods_price,goods_promotion_price,goods_promotion_type,goods_marketprice,goods_image,goods_salenum,evaluation_good_star,evaluation_count,pf,ifpf";

        $fieldstr .= ',is_virtual,is_presell,is_fcode,have_gift,goods_jingle,store_id,store_name,is_own_mall';
        ...
        if (!is_null($goods_list)) {
            ...
            foreach ($goods_list as $key => $value) {
                if($value['ifpf']==1){
                    ...
                    if($pf['q1']>0 && $pf['f1']){
                        $goods_list[$key]['q1'] = $pf['q1'];
                        $goods_list[$key]['f1'] = number_format($pf['f1'], 2);
                    }
                    if($pf['q2']>0 && $pf['f2']){
                         $goods_list[$key]['q2'] = $pf['q2'];
                        $goods_list[$key]['f2'] = number_format($pf['f2'], 2);
                    }
                    if($pf['q3']>0 && $pf['f3']){
                        $goods_list[$key]['q3'] = $pf['q3'];
                        $goods_list[$key]['f3'] = number_format($pf['f3'], 2);
                    }
                }
            }
    ...
    private function _goods_list_order($key, $order) {
        $result = 'is_own_mall desc,goods_id desc';
        if (!empty($key)) {
     ...
     * 商品详细页
     */
    public function goods_detailFeiwa() {
        ...
        $goods_detail['store_info']['is_own_mall'] = $store_info['is_own_mall'];
		...
		//批发
		$goods_detail['goods_info']['pf']=unserialize($goods_detail['goods_info']['pf']);
    ...
    private function _goods_detail_extend($goods_detail) {
        ...
        //商品链接
        $goods_detail['goods_info']['goods_url'] = urlMall('goods', 'index', array('goods_id' => $goods_detail['goods_info']['goods_id']));
     ...
     * 商品详细页
     */
    public function goods_bodyFeiwa() {
    ...
    public function goods_evaluateFeiwa() {
        $goods_id = intval($_GET['goods_id']);
     ...
     * 取得店铺分店地址
     */
    public function store_o2o_addrFeiwa() {
    ...
    public function auto_completeFeiwa() {
