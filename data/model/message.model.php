...
class messageModel extends Model {
    ...
    public function saveMessage($param) {
        ...
        $array['is_level']      = $param['is_level']?$param['is_level']:'';
    ...
    private function getCondition($condition_array){
        ...
        if($condition_array['to_member_id'] != ''){
            $condition_sql  .= " and (message.to_member_id ='all' or (message.message_ismore=0 and message.to_member_id='{$condition_array['to_member_id']}') or (message.message_ismore=1 and message.to_member_id like '%,{$condition_array['to_member_id']},%') or (message.to_member_id='level' and message.is_level='{$condition_array['is_level']}'))";
        }
        /////会员leverl
        if($condition_array['member_exppoints'] != ''){
           // $condition_sql  .= " or (message.to_member_id='level' and message.is_level>='{$condition_array['member_exppoints']}')";
        }
        //发信人
