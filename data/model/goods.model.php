...
class goodsModel extends Model{
	...
	*获取批发价格 
	**/
    public function getpf($goods_id,$quantity){
	$result=$this->getGoodsInfo(array('goods_id'=>$goods_id));
	
    if($result['pf']){
	     $pf=unserialize($result['pf']);
		
       if ($quantity >= intval($pf['q3']) && intval($pf['q3'])>0) {
            $pfprice = ncPriceFormat($pf['f3']);
            
        }elseif($quantity >= intval($pf['q2'])&& intval($pf['q2'])>0){
		
		$pfprice = ncPriceFormat($pf['f2']);
		
		}elseif($quantity >= intval($pf['q1']) && intval($pf['q1'])>0){

        $pfprice = ncPriceFormat($pf['f1']);
		}else{
		
		$pfprice = $result['goods_price'];
		}
	...
	**/
    ...
    public function getifpf($goods_id,$price){
		
	$result=$this->getGoodsInfo(array('goods_id'=>$goods_id));
	   if($result['pf']){
	     $pf=unserialize($result['pf']);
		 
	      if($price== ncPriceFormat($pf['f1'])|| $price==  ncPriceFormat($pf['f2'])|| $price== ncPriceFormat($pf['f3'])){
    ...
    public function getGoodsListByColorDistinct($condition, $field = '*', $order = 'goods_id asc', $page = 0, $limit = 0) {
        ...
        $goods_list = array();
        if (C('dbdriver') == 'mysql') {
            $goods_list = $this->getGoodsOnlineList($condition, $_field.' feiwa_distinct,'.$field, $page, $order, $limit, $_distinct, false, $count);
        } elseif (C('dbdriver') == 'oracle') {
    ...
    public function getGoodsDetail($goods_id) {
        ...
        foreach ($spec_array as $key => $value) {
            ...
            $tpl_spec['url'] = urlMall('goods', 'index', array('goods_id' => $value['goods_id']));
            $spec_list[] = $tpl_spec;
...
//      }
 ...
 // 新版放大镜开始
     ...
     * 按商品编号查询商品的信息
     *
     * @param array $goodsid_array 店铺编号
     * @return array
     */
    public function getGoodsIDList($goodsid_array, $field = '*') {
