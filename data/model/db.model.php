 ...
 * md5 加密后缀
 * 
 * @copyright  Copyright (c) 2007-2013 FeiWa   (#)
...
class dbModel{
	...
	public function backUp($step=1){
		 ...
		 * 创建表sql
		 */
		if ($_SESSION['db_backup']['feiwa'] == 'create'){
		 ...
		 * 表信息sql
		 */
		if ($_SESSION['db_backup']['feiwa'] == 'insert'){
			foreach ($table_list as $k => $v){
				/**
				 * 写入session
				 */
	...
	public function getBackDir(){
		 ...
		 * 按照日期进行生成
		 */
		$dir_list = readDirList(BASE_DATA_PATH.DS.'feiwa_sql_back');
		$tmp = date('Ymd');
		$check_array = array();
	...
	public function writeBackFile(){
		...
		$step = $this->step;
		try {
			if (!is_dir(BASE_DATA_PATH.DS.'feiwa_sql_back'.DS.$_SESSION['db_backup']['back_file'])){
				if (!$this->mkdirs(BASE_DATA_PATH.DS.'feiwa_sql_back'.DS.$_SESSION['db_backup']['back_file'],0755)){
					$error = $lang['db_backup_mkdir_fail'];
					throw new Exception($error);
				}else {
					$fp = @fopen(BASE_DATA_PATH.DS.'feiwa_sql_back'.DS.$_SESSION['db_backup']['back_file'].DS.'index.html','w+');
					@fclose($fp);
				}
			}
			$file_name = BASE_DATA_PATH.DS.'feiwa_sql_back'.DS.$_SESSION['db_backup']['back_file'].DS.$_SESSION['db_backup']['back_file'].'_'.$step.'_'.$_SESSION['db_backup']['md5'].'.sql';
	...
	public function import($path,$step=1){
		$dir = BASE_DATA_PATH.DS.'feiwa_sql_back'.DS.$path;
		$file_list = array();
