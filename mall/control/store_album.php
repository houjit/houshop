...
class store_albumControl extends BaseSellerControl {
     ...
     *
     */
    public function album_cateFeiwa(){
...
//      Tpl::output('show_page',$page->show());
     ...
     *
     */
    public function album_addFeiwa(){
        /**
         * 实例化相册模型
         */
        $model_album = Model('album');
     ...
     *
     */
    public function album_add_saveFeiwa(){
        if (chksubmit()){
            /**
             * 实例化相册模型
             */
            ...
            if($class_count['count'] >= 20){
                showDialog(Language::get('album_class_save_max_20'),'index.php?app=store_album','error',empty($_GET['inajax'])?'':'CUR_DIALOG.close();');
            }
            /**
             * 实例化相册模型
             */
            ...
            if($return){
                showDialog(Language::get('album_class_save_succeed'),'index.php?app=store_album','succ',empty($_GET['inajax'])?'':'CUR_DIALOG.close();');
            }
        }
     ...
     * 相册分类编辑
     */
    public function album_editFeiwa(){
        if(empty($_GET['id'])){
     ...
     * 相册分类编辑保存
     */
    public function album_edit_saveFeiwa(){
        $param = array();
        $param['aclass_name']   = $_POST['name'];
        $param['aclass_des']    = $_POST['description'];
        $param['aclass_sort']   = intval($_POST['sort']);
        ...
        if($return){
            ...
            if($re){
                showDialog(Language::get('album_class_edit_succeed'),'index.php?app=store_album','succ',empty($_GET['inajax'])?'':'CUR_DIALOG.close();');
            }
        }else{
     ...
     * 相册删除
     */
    public function album_delFeiwa(){
        if(empty($_GET['id'])){
        ...
        if($return){
            showDialog(Language::get('album_class_file_del_succeed'),'index.php?app=store_album','succ');
        }else{
            showDialog(Language::get('album_class_file_del_lose'));
        }
    }
     ...
     * 图片列表
     */
    public function album_pic_listFeiwa(){
        if(empty($_GET['id'])) {
            showMessage(Language::get('album_parameter_error'),'','html','error');
        }
     ...
     * 修改相册封面
     */
    public function change_album_coverFeiwa(){
        if(empty($_GET['id'])) {
     ...
     * ajax修改图名称
     */
    public function change_pic_nameFeiwa(){
     ...
     * 图片详细页
     */
    public function album_pic_infoFeiwa(){
     ...
     * 图片 ajax
     */
    public function album_ad_ajaxFeiwa(){
        if(empty($_GET['class_id']) && empty($_GET['id'])){
            exit();
        }
     ...
     * 图片删除
     */
    public function album_pic_delFeiwa(){
     ...
     * 移动相册
     */
    public function album_pic_moveFeiwa(){
        /**
         * 实例化相册类
         */
        $model_album = Model('album');
     ...
     * 替换图片
     */
    public function replace_image_uploadFeiwa() {
        $file = $_GET['id'];
	 ...
	 * 添加水印
	 */
	public function album_pic_watermarkFeiwa(){
		if(empty($_POST['id']) && !is_array($_POST['id'])) {
			showDialog(Language::get('album_parameter_error'));
		}
		...
		if ($store_wm_info['wm_image_name'] == '' && $store_wm_info['wm_text'] == ''){
			showDialog(Language::get('album_class_setting_wm'),"index.php?app=store_album&feiwa=store_watermark",'error','CUR_DIALOG.close();');//"请先设置水印"
		}
     ...
     * 水印管理
     */
    public function store_watermarkFeiwa(){
        /**
         * 读取语言包
         */
    ...
    private function profile_menu($menu_type,$menu_key=''){
        ...
        switch ($menu_type) {
            case 'album':
                $menu_array = array(
                1=>array('menu_key'=>'album','menu_name'=>Language::get('feiwa_member_path_my_album'),'menu_url'=>'index.php?app=store_album'),
                2=>array('menu_key'=>'watermark','menu_name'=>Language::get('feiwa_member_path_watermark'),'menu_url'=>'index.php?app=store_album&feiwa=store_watermark')
                );
                break;
            case 'album_pic':
                $menu_array = array(
                1=>array('menu_key'=>'album','menu_name'=>Language::get('feiwa_member_path_my_album'),'menu_url'=>'index.php?app=store_album'),
                3=>array('menu_key'=>'pic_list','menu_name'=>Language::get('feiwa_member_path_album_pic_list'),'menu_url'=>'index.php?app=store_album&feiwa=album_pic_list&id='.intval($_GET['id'])),
                2=>array('menu_key'=>'watermark','menu_name'=>Language::get('feiwa_member_path_watermark'),'menu_url'=>'index.php?app=store_album&feiwa=store_watermark')
                );
                break;
            case 'album_pic_info':
                $menu_array = array(
                1=>array('menu_key'=>'album','menu_name'=>Language::get('feiwa_member_path_my_album'),'menu_url'=>'index.php?app=store_album'),
                3=>array('menu_key'=>'pic_info','menu_name'=>Language::get('feiwa_member_path_album_pic_info'),'menu_url'=>'index.php?app=store_album&feiwa=album_pic_info&id='.intval($_GET['id']).'&class_id='.intval($_GET['class_id'])),
                2=>array('menu_key'=>'watermark','menu_name'=>Language::get('feiwa_member_path_watermark'),'menu_url'=>'index.php?app=store_album&feiwa=store_watermark')
                );
                break;
        }
     ...
     * ajax返回图片信息
     */
    public function ajax_change_imgmessageFeiwa(){
     ...
     * ajax验证名称时候重复
     */
    public function ajax_check_class_nameFeiwa(){
