...
class store_voucherControl extends BaseSellerControl{
    ...
    public function __construct() {
        ...
        if (C('voucher_allow') != 1){
            showMessage(Language::get('voucher_unavailable'),'index.php?app=store','html','error');
        }
        //申请记录状态
     ...
     * 代金券模版列表
     */
    public function voucherlistFeiwa(){
     ...
     * 导出
     */
    public function voucher_exportFeiwa(){
        $t_id = intval($_GET['tid']);
        if($t_id <= 0){
            showDialog('参数错误');
        }
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){//显示下载链接
                ...
                Tpl::output('murl','index.php?app=store_voucher&feiwa=voucher_export');
    ...
    public function quotaaddFeiwa(){
        if (chksubmit()){
            ...
            $this->recordSellerLog('购买'.$quota_quantity.'份代金券套餐，单价'.$current_price.L('feiwa_yuan'));

            if($reault){
                showDialog(Language::get('voucher_apply_buy_succ'),'index.php?app=store_voucher&feiwa=templatelist','succ');
            } else {
                showDialog(Language::get('feiwa_common_op_fail'),'index.php?app=store_voucher&feiwa=templatelist');
            }
        }else {
            //输出导航
     ...
     * 代金券模版添加
     */
    public function templateaddFeiwa(){
        $model = Model('voucher');
        if ($isOwnMall = checkPlatformStore()) {
            Tpl::output('isOwnMall', true);
        } else {
            //查询当前可以套餐
            $quotainfo = $model->getCurrentQuota($_SESSION['store_id']);
            if(empty($quotainfo)){
                showMessage(Language::get('voucher_template_quotanull'),'index.php?app=store_voucher&feiwa=quotaadd','html','error');
            }
            ...
            if ($count >= C('promotion_voucher_storetimes_limit')){
                ...
                showMessage($message,'index.php?app=store_voucher&feiwa=templatelist','html','error');
            }
        }
        ...
        if(empty($pricelist)){
            showMessage(Language::get('voucher_template_pricelisterror'),'index.php?app=store_voucher&feiwa=templatelist','html','error');
        }
        //获取领取方式
        ...
        if(chksubmit()){
            ...
            if ($error){
                ...
                if ($_POST['txt_template_enddate']){
                    ...
                    if (!$isOwnMall && $enddate > $quotainfo['quota_endtime']){
                ...
                if($rs){
                    ...
                    showDialog(Language::get('feiwa_common_save_succ'),'index.php?app=store_voucher&feiwa=templatelist','succ');
                }else{
                    showDialog(Language::get('feiwa_common_save_fail'),'index.php?app=store_voucher&feiwa=templatelist','error');
                }
            }
        }else{
            //店铺分类
    ...
    public function templateeditFeiwa(){
        ...
        if ($t_id <= 0){
            showMessage(Language::get('wrong_argument'),'index.php?app=store_voucher&feiwa=templatelist','html','error');
        }
        $model = Model('voucher');
        //查询模板信息
        $param = array();
        ...
        if(chksubmit()){
            ...
            if ($error){
                ...
                if($rs){
                    ...
                    showDialog(Language::get('feiwa_common_op_succ'),'index.php?app=store_voucher&feiwa=templatelist','succ');
                }else{
                    showDialog(Language::get('feiwa_common_op_fail'),'index.php?app=store_voucher&feiwa=templatelist','error');
                }
            }
        }else{
     ...
     * 删除代金券
     */
    public function templatedelFeiwa(){
        $t_id = intval($_GET['tid']);
        if ($t_id <= 0){
            showMessage(Language::get('wrong_argument'),'index.php?app=store_voucher&feiwa=templatelist','html','error');
        }
        ...
        if ($rs){
            ...
            showDialog(Language::get('feiwa_common_del_succ'),'reload','succ');
        }else {
     ...
     * 查看代金券详细
     */
    public function templateinfoFeiwa(){
        $t_id = intval($_GET['tid']);
        if ($t_id <= 0){
            showMessage(Language::get('wrong_argument'),'index.php?app=store_voucher&feiwa=templatelist','html','error');
        }
     ...
     * 生成卡密代金券
     */
    public function bulidvoucherFeiwa(){
        $t_id = intval($_GET['tid']);
    	if ($t_id <= 0){
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            case 'voucher':
                $menu_array = array(
                1=>array('menu_key'=>'templatelist','menu_name'=>Language::get('feiwa_member_path_store_voucher'), 'menu_url'=>'index.php?app=store_voucher&feiwa=templatelist'),
                );
                break;
            case 'quota_add':
                $menu_array = array(
                1=>array('menu_key'=>'templatelist','menu_name'=>Language::get('feiwa_member_path_store_voucher'), 'menu_url'=>'index.php?app=store_voucher&feiwa=templatelist'),
                4=>array('menu_key'=>'quotaadd','menu_name'=>Language::get('voucher_applyadd'), 'menu_url'=>'index.php?app=store_voucher&feiwa=quotaadd')
                );
                break;
            case 'template':
                $menu_array = array(
                1=>array('menu_key'=>'templatelist','menu_name'=>Language::get('feiwa_member_path_store_voucher'), 'menu_url'=>'index.php?app=store_voucher&feiwa=templatelist'),
                2=>array('menu_key'=>'templateadd','menu_name'=>Language::get('voucher_templateadd'),   'menu_url'=>'index.php?app=store_voucher&feiwa=templateadd'),
                );
                break;
            case 'templateedit':
                $menu_array = array(
