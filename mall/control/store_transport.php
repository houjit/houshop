...
class store_transportControl extends BaseSellerControl {
    ...
    public function saveFeiwa(){
        ...
        if ($result){
            header('location: index.php?app=store_transport&type='.$_POST['type']);exit;
        }else{
            showMessage(Language::get('transport_op_fail'),$_SERVER['HTTP_REFERER'],'html','error');
        }
    }
     ...
     *
     */
    public function offpay_areaFeiwa()
    {
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            ...
            case 'offpay_area':
                $menu_array = array(
                1=>array('menu_key'=>'transport',   'menu_name'=>Language::get('feiwa_member_path_postage'),       'menu_url'=>'index.php?app=store_transport'),
                );
                if (checkPlatformStore()) {
                    $menu_array[] = array(
                        'menu_key' => 'offpay_area',
                        'menu_name' => '货到付款地区',
                        'menu_url' => 'index.php?app=store_transport&feiwa=offpay_area'
                    );
                }
                break;
        }
