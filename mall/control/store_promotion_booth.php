...
class store_promotion_boothControl extends BaseSellerControl {
    ...
    public function indexFeiwa() {
        $this->booth_goods_listFeiwa();
    }

    /**
     * 套餐商品列表
     */
    public function booth_goods_listFeiwa() {
     ...
     * 选择商品
     */
    public function booth_select_goodsFeiwa() {
     ...
     * 购买套餐
     */
    public function booth_quota_addFeiwa() {
        if (chksubmit()) {
            ...
            if ($quantity <= 0 || $quantity > 12) {
                showDialog('参数错误，购买失败。', urlMall('store_promotion_booth', 'booth_quota_add'), '', 'error' );
            }
            // 实例化模型
            $model_booth = Model('p_booth');
            ...
            if ($return) {
                ...
                showDialog('购买成功', urlMall('store_promotion_booth', 'booth_goods_list'), 'succ');
            } else {
                showDialog('购买失败', urlMall('store_promotion_booth', 'booth_quota_add'));
            }
        }
        // 输出导航
     ...
     * 套餐续费
     */
    public function booth_renewFeiwa() {
        if (chksubmit()) {
            $model_booth = Model('p_booth');
            $quantity = intval($_POST['booth_quota_quantity']); // 购买数量（月）
            $price_quantity = $quantity * intval(C('promotion_booth_price')); // 扣款数
            ...
            if ($return) {
                ...
                showDialog('购买成功', urlMall('store_promotion_booth', 'booth_list'), 'succ');
            } else {
                showDialog('购买失败', urlMall('store_promotion_booth', 'booth_quota_add'));
            }
        }
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            case 'booth_goods_list':
                $menu_array = array(
                    1=>array('menu_key'=>'booth_goods_list', 'menu_name'=>'商品列表', 'menu_url'=>urlMall('store_promotion_booth', 'booth_goods_list'))
                );
                break;
            case 'booth_quota_add':
                $menu_array = array(
                    1=>array('menu_key'=>'booth_goods_list', 'menu_name'=>'商品列表', 'menu_url'=>urlMall('store_promotion_booth', 'booth_goods_list')),
                    2=>array('menu_key'=>'booth_quota_add', 'menu_name'=>'购买套餐', 'menu_url'=>urlMall('store_promotion_booth', 'booth_quota_add'))
                );
                break;
            case 'booth_renew':
                $menu_array = array(
                    1=>array('menu_key'=>'booth_goods_list', 'menu_name'=>'商品列表', 'menu_url'=>urlMall('store_promotion_booth', 'booth_goods_list')),
                    2=>array('menu_key'=>'booth_renew', 'menu_name'=>'套餐续费', 'menu_url'=>urlMall('store_promotion_booth', 'booth_renew'))
                );
                break;
        }
