...
class member_refundControl extends BaseMemberControl {
     ...
     *
     */
    public function add_refundFeiwa(){
        $model_refund = Model('refund_return');
        $condition = array();
        ...
        if ($order_id < 1 || $goods_id < 1) {//参数验证
            showDialog(Language::get('wrong_argument'),'index.php?app=member_order&feiwa=index','error');
        }
        $condition = array();
        ...
        if (chksubmit() && $goods_id > 0){
            ...
            $show_url = 'index.php?app=member_return&feiwa=index';
            ...
            if ($refund_array['refund_type'] != '2') {
                ...
                $show_url = 'index.php?app=member_refund&feiwa=index';
            }
            ...
            if ($state) {
                ...
                showDialog(Language::get('feiwa_common_save_succ'),$show_url,'succ');
            } else {
     ...
     *
     */
    public function add_refund_allFeiwa(){
        ...
        if (chksubmit()) {
            ...
            if ($state) {
                $model_refund->editOrderLock($order_id);
                showDialog(Language::get('feiwa_common_save_succ'),'index.php?app=member_refund&feiwa=index','succ');
            } else {
                showDialog(Language::get('feiwa_common_save_fail'),'reload','error');
            }
        }
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            case 'member_order':
                $menu_array = array(
                array('menu_key'=>'buyer_refund','menu_name'=>Language::get('feiwa_member_path_buyer_refund'), 'menu_url'=>'index.php?app=member_refund'),
                array('menu_key'=>'buyer_return','menu_name'=>Language::get('feiwa_member_path_buyer_return'), 'menu_url'=>'index.php?app=member_return'),
                array('menu_key'=>'buyer_vr_refund','menu_name'=>'虚拟兑码退款',    'menu_url'=>'index.php?app=member_vr_refund'));
                break;
        }
