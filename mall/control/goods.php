...
class goodsControl extends BaseGoodsControl {
    ...
    public function indexFeiwa() {
        ...
        // 看了又看（同分类本店随机商品）
        $size = $goods_info['is_own_mall'] ? 6 : 6;
     ...
     * 记录浏览历史
     */
    public function addbrowseFeiwa(){
	 ...
	 * 置空交流
	 */
	 public function callcenterFeiwa(){
        ...
        $this->outputStoreInfo($store_info, $goods_info);
	 	 ...
	 	 Tpl::showpage('callcenter','null_layout');
	 }
     ...
     * 商品评价详细页
     */
    public function comments_listFeiwa() {
        $goods_id = intval($_GET['goods_id']);
        ...
        // 当前位置导航
        $nav_link_list = Model('goods_class')->getGoodsClassNav($goods_info['gc_id'], 0);
        $nav_link_list[] = array('title' => $goods_info['goods_name'], 'link' => urlMall('goods', 'index', array('goods_id' => $goods_id)));
     ...
     * 产品咨询
     */
    public function consulting_listFeiwa() {
     ...
     * 商品咨询添加
     */
    public function save_consultFeiwa(){
        //检查是否可以评论
        ...
        if (process::islock('commit')){
            showDialog(L('feiwa_common_op_repeat'));
        }else{
     ...
     * 异步显示优惠套装/推荐组合
     */
    public function get_bundlingFeiwa() {
        $goods_id = intval($_GET['goods_id']);
        if ($goods_id <= 0) {
            exit();
        }
     ...
     * 到货通知
     */
    public function arrival_noticeFeiwa() {
     ...
     * 到货通知表单
     */
    public function arrival_notice_submitFeiwa() {
        ...
        $js = "ajax_form('arrival_notice', '". $title ."', '" . urlMall('goods', 'arrival_notice_succ', array('type' => $type)) . "', 480);";
        showDialog('','','js',$js);
    }
     ...
     * 到货通知添加成功
     */
    public function arrival_notice_succFeiwa() {
        // 可能喜欢的商品
     ...
     * 显示门店
     */
    public function show_chainFeiwa() {
