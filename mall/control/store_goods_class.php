...
class store_goods_classControl extends BaseSellerControl {
     ...
     * @return
     */
    public function goods_class_saveFeiwa() {
        $model_class    = Model('store_goods_class');
        if(isset($_POST['stc_id'])) {
            ...
            if($state) {
                showDialog(Language::get('feiwa_common_save_succ'),'index.php?app=store_goods_class&feiwa=index','succ',empty($_GET['inajax']) ?'':'CUR_DIALOG.close();');
            } else {
                showDialog(Language::get('feiwa_common_save_fail'));
            }
        } else {
            $class_array        = array();
            $class_array['stc_name']      = $_POST['stc_name'];
     ...
     * @return
     */
    public function drop_goods_classFeiwa() {
        $model_class    = Model('store_goods_class');
        ...
        if ($drop_state){
            showDialog(Language::get('feiwa_common_del_succ'),'index.php?app=store_goods_class&feiwa=store_goods_class','succ');
        }else{
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            case 'store_goods_class':
                $menu_array = array(
                1=>array('menu_key'=>'store_goods_class','menu_name'=>'店铺分类',   'menu_url'=>'index.php?app=store_goods_class&feiwa=store_goods_class'));
                break;
        }
