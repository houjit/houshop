...
class store_billControl extends BaseSellerControl {
     ...
     *
     */
    public function confirm_billFeiwa(){
        if (!preg_match('/^\d+$/',$_GET['ob_id'])) {
        ...
        if ($update){
            ...
            showDialog(L('feiwa_common_op_fail'),'reload','error');
        }
    }
    ...
    public function export_orderFeiwa(){
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){
                ...
                Tpl::output('murl','index.php?app=store_bill&feiwa=show_bill&ob_id='.$_GET['ob_id']);
    ...
    public function export_refund_orderFeiwa(){
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){   //显示下载链接
                ...
                Tpl::output('list',$array);
                Tpl::output('murl','index.php?app=store_bill&feiwa=show_bill&query_type=refund&ob_id='.$_GET['ob_id']);
                Tpl::showpage('store_export.excel');
                exit();
            }else{
                //如果数量小，直接下载
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            case 'list':
                $menu_array = array(
                1=>array('menu_key'=>'list','menu_name'=>'实物订单结算', 'menu_url'=>'index.php?app=bill&feiwa=list'),
                );
                break;
            case 'show':
                $menu_array = array(
                array('menu_key'=>'order_list','menu_name'=>'订单列表', 'menu_url'=>'index.php?app=store_bill&feiwa=show_bill&ob_id='.$_GET['ob_id']),
                array('menu_key'=>'refund_list','menu_name'=>'退款订单','menu_url'=>'index.php?app=store_bill&feiwa=show_bill&type=refund&ob_id='.$_GET['ob_id']),
                array('menu_key'=>'cost_list','menu_name'=>'促销费用','menu_url'=>'index.php?app=store_bill&feiwa=show_bill&type=cost&ob_id='.$_GET['ob_id'])
                );
                if (floatval($this->_bill_info['ob_order_book_totals']) > 0) {
                    array_push($menu_array,array('menu_key'=>'book_list','menu_name'=>'未退定金', 'menu_url'=>'index.php?app=store_bill&feiwa=show_bill&type=book&ob_id='.$_GET['ob_id']));
                }
                break;
        }
