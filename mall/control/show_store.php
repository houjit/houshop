...
class show_storeControl extends BaseStoreControl {
    ...
    public function indexFeiwa(){
        if(!$this->store_decoration_only) {
			...
			//评价信息
			$storesid= array();
			$storesid['geval_storeid'] = $this->store_info['store_id'];
             $goods_evaluate_info = Model('evaluate_goods')->getEvaluateGoodsList($storesid,'',6);
    ...
    public function show_articleFeiwa() {
        //判断是否为导航页面
     ...
     * 全部商品
     */
    public function goods_allFeiwa(){
     ...
     * 用户口碑列表页面
     */
    function comments_goodsFeiwa() {
    	
		$store_id = $this->store_info['store_id'];
        $this->_get_comments($store_id, 20);
        
        Tpl::output('page','store_comments_goods');

        Tpl::showpage('show.comments');
    }
	
	 private function _get_comments($store_id, $page) {
        $condition = array();
		...
		foreach($goodsevallist as $k=>$v){
			$goodsevallist[$k]['credit_percent']=intval($v['geval_scores'] / 5 * 100);
		}
     ...
     * ajax获取动态数量
     */
    function ajax_store_trend_countFeiwa(){
     ...
     * ajax 店铺流量统计入库
     */
    public function ajax_flowstat_recordFeiwa(){
