...
class store_deliverControl extends BaseSellerControl {
     ...
     * @return boolean
     */
    public function buyer_address_editFeiwa() {
        $order_id = intval($_GET['order_id']);
     ...
     * 收货地址保存
     */
    public function buyer_address_saveFeiwa() {
        $model_order = Model('order');
        $data = array();
     ...
     * @return boolean
     */
    public function send_address_selectFeiwa() {
     ...
     * 保存发货地址修改
     */
    public function send_address_saveFeiwa() {
     ...
     * 延迟收货
     */
    public function delay_receiveFeiwa(){
        $order_id = intval($_GET['order_id']);
     ...
     * 运单打印
     */
    public function waybill_printFeiwa() {
        $order_id = intval($_GET['order_id']);
        if($order_id <= 0) {
        ...
        if(empty($store_waybill_info)) {
            showMessage('请首先绑定打印模板', urlMall('store_waybill', 'waybill_manage'), '', 'error');
        }
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            case 'deliver':
                $menu_array = array(
                array('menu_key'=>'deliverno',          'menu_name'=>Language::get('feiwa_member_path_deliverno'), 'menu_url'=>'index.php?app=store_deliver&feiwa=index&state=deliverno'),
                array('menu_key'=>'delivering',         'menu_name'=>Language::get('feiwa_member_path_delivering'),    'menu_url'=>'index.php?app=store_deliver&feiwa=index&state=delivering'),
                array('menu_key'=>'delivered',      'menu_name'=>Language::get('feiwa_member_path_delivered'), 'menu_url'=>'index.php?app=store_deliver&feiwa=index&state=delivered'),
                );
                break;
            case 'search':
                $menu_array = array(
                1=>array('menu_key'=>'nodeliver',           'menu_name'=>Language::get('feiwa_member_path_deliverno'), 'menu_url'=>'index.php?app=store_deliver&feiwa=index&state=nodeliver'),
                2=>array('menu_key'=>'delivering',          'menu_name'=>Language::get('feiwa_member_path_delivering'),    'menu_url'=>'index.php?app=store_deliver&feiwa=index&state=delivering'),
                3=>array('menu_key'=>'delivered',       'menu_name'=>Language::get('feiwa_member_path_delivered'), 'menu_url'=>'index.php?app=store_deliver&feiwa=index&state=delivered'),
                4=>array('menu_key'=>'search',      'menu_name'=>Language::get('feiwa_member_path_deliver_info'),  'menu_url'=>'###'),
                );
                break;
        }
