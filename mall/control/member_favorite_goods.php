...
class member_favorite_goodsControl extends BaseMemberControl{
     ...
     * 增加商品收藏
     */
    public function favoritegoodsFeiwa(){
     ...
     * @return
     */
    public function delfavoritesFeiwa(){
        ...
        if (!empty($fav_arr) && is_array($fav_arr)){
            ...
            if (!empty($favorites_list) && is_array($favorites_list)){
                ...
                if (!empty($fav_arr) && $result){
                    ...
                    showDialog(Language::get('favorite_del_success'),'index.php?app=member_favorite_goods&feiwa=fglist&show='.$_GET['show'],'succ');
                }
            }else {
    ...
    public function log_msgFeiwa(){
        ...
        if (chksubmit()) {
            ...
            if ($state) {
                showDialog(Language::get('feiwa_common_save_succ'),'reload','succ','CUR_DIALOG.close();');
            } else {
                showDialog(Language::get('feiwa_common_save_fail'),'reload','error','CUR_DIALOG.close();');
            }
        }
    ...
    private function profile_menu($menu_type,$menu_key='') {
        $menu_array = array(
            array('menu_key'=>'fav_goods','menu_name'=>Language::get('feiwa_member_path_collect_list'), 'menu_url'=>'index.php?app=member_favorite_goods&feiwa=fglist'),
            array('menu_key'=>'fav_goodsstore','menu_name'=>'同店商品', 'menu_url'=>'index.php?app=member_favorite_goods&feiwa=fglist&show=store'),
        );
