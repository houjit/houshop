...
class store_returnControl extends BaseSellerControl {
    ...
    public function editFeiwa() {
        ...
        if (chksubmit()) {
            $reload = 'index.php?app=store_return&lock=1';
            if ($return['order_lock'] == '2') {
                $reload = 'index.php?app=store_return&lock=2';
            }
            ...
            if ($state) {
                ...
                $param['param'] = array(
                    'refund_url' => urlMall('member_return', 'view', array('return_id' => $return['refund_id'])),
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            case 'return':
                $menu_array = array(
                    array('menu_key'=>'2','menu_name'=>'售前退货',  'menu_url'=>'index.php?app=store_return&lock=2'),
                    array('menu_key'=>'1','menu_name'=>'售后退货','menu_url'=>'index.php?app=store_return&lock=1')
                );
                break;
        }
