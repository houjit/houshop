...
class buyControl extends BaseBuyControl {
     ...
     */
    public function buy_step1Feiwa() {
     ...
     *
     */
    public function buy_step2Feiwa() {
        $logic_buy = logic('buy');
        ...
        if(!$result['state']) {
            showMessage($result['msg'], 'index.php?app=cart', 'html', 'error');
        }

        //转向到商城支付页面
        redirect('index.php?app=buy&feiwa=pay&pay_sn='.$result['data']['pay_sn']);
    }
    ...
    public function payFeiwa() {
        ...
        if (!preg_match('/^\d{18}$/',$pay_sn)){
            showMessage(Language::get('cart_order_pay_not_exists'),'index.php?app=member_order','html','error');
        }
        ...
        if (empty($order_list)) {
            showMessage('未找到需要支付的订单','index.php?app=member_order','html','error');
        }
        ...
        if (empty($pay['pay_amount_online']) && empty($pay['pay_amount_offline'])) {
            redirect('index.php?app=buy&feiwa=pay_ok&pay_sn='.$pay_sn.'&is_chain='.$flag_chain.'&pay_amount='.ncPriceFormat($pay_amount));
        }
     ...
     *
     */
    public function load_addrFeiwa() {
     ...
     * 载入门店自提点
     */
    public function load_chainFeiwa() {
     ...
     */
    public function change_addrFeiwa() {
        $logic_buy = Logic('buy');
    ...
    //根据门店自提站ID计算商品库存
    public function change_chainFeiwa() {
        $logic_buy = Logic('buy');
      ...
      *
      */
     public function add_chainFeiwa(){
    ...
    public function load_invFeiwa() {
      ...
      *
      */
     public function add_invFeiwa(){
        $model_inv = Model('invoice');
        if (chksubmit()){
            //如果是增值税发票验证表单信息
            if ($_POST['invoice_type'] == 2) {
                if (empty($_POST['inv_company']) || empty($_POST['inv_code']) || empty($_POST['inv_reg_addr'])) {
                    exit(json_encode(array('state'=>false,'msg'=>Language::get('feiwa_common_save_fail','UTF-8'))));
                }
            }
            $data = array();
            ...
            if ($insert_id) {
                ...
                exit(json_encode(array('state'=>'fail','msg'=>Language::get('feiwa_common_save_fail','UTF-8'))));
            }
        } else {
            Tpl::showpage('buy_address.add','null_layout');
        }
     ...
     * AJAX验证支付密码
     */
    public function check_pd_pwdFeiwa(){
     ...
     * F码验证
     */
    public function check_fcodeFeiwa() {
    ...
    private function _buy_branch($post) {
        if (!$post['ifcart']) {
            ...
            if ($goods_info['is_virtual']) {
                redirect('index.php?app=buy_virtual&feiwa=buy_step1&goods_id='.$goods_id.'&quantity='.$quantity);
            }
        }
    }
