...
class store_promotion_mansongControl extends BaseSellerControl {
    ...
    public function __construct() {
        ...
        if (intval(C('promotion_allow')) !== 1) {
            showMessage(Language::get('promotion_unavailable'),'index.php?app=seller_center','','error');
        }

    }

    public function indexFeiwa() {
        $this->mansong_listFeiwa();
    }

    /**
     * 发布的满就送活动列表
     **/
    public function mansong_listFeiwa() {
     ...
     * 添加满就送活动
     **/
    public function mansong_addFeiwa() {
        $model_mansong_quota = Model('p_mansong_quota');
        $model_mansong = Model('p_mansong');
     ...
     * 保存添加的满就送活动
     **/
    public function mansong_saveFeiwa() {
        ...
        if($mansong_id) {
            ...
            showDialog(Language::get('mansong_add_success'), urlMall('store_promotion_mansong', 'mansong_list'), 'succ');
        } else {
     ...
     * 满就送活动详细信息
     **/
    public function mansong_detailFeiwa() {
     ...
     * 满就送活动删除
     **/
    public function mansong_delFeiwa() {
        ...
        if($result) {
            ...
            showDialog(L('feiwa_common_op_succ'), urlMall('store_promotion_mansong', 'mansong_list'), 'succ');
        } else {
     ...
     * 满就送套餐购买
     **/
    public function mansong_quota_addFeiwa() {
     ...
     * 满就送套餐购买保存
     **/
    public function mansong_quota_add_saveFeiwa() {
        ...
        $this->recordSellerLog('购买'.$mansong_quota_quantity.'份满即送套餐，单价'.$current_price.L('feiwa_yuan'));

        showDialog(Language::get('mansong_quota_add_success'), urlMall('store_promotion_mansong', 'mansong_list'), 'succ');
    }
    ...
    private function profile_menu($menu_key='') {
        $menu_array = array(
            1=>array('menu_key'=>'mansong_list','menu_name'=>Language::get('promotion_active_list'),'menu_url'=>urlMall('store_promotion_mansong', 'mansong_list')),
        );
        switch ($menu_key){
            case 'mansong_add':
                $menu_array[] = array('menu_key'=>'mansong_add','menu_name'=>Language::get('promotion_join_active'),'menu_url'=>urlMall('store_promotion_mansong', 'mansong_add'));
                break;
            case 'mansong_quota_add':
                $menu_array[] = array('menu_key'=>'mansong_quota_add','menu_name'=>Language::get('promotion_buy_product'),'menu_url'=>urlMall('store_promotion_mansong', 'mansong_quota_add'));
                break;
            case 'mansong_detail':
                $menu_array[] = array('menu_key'=>'mansong_detail','menu_name'=>Language::get('mansong_active_content'),'menu_url'=>urlMall('store_promotion_mansong', 'mansong_detail', array('mansong_id' => $_GET['mansong_id'])));
                break;
        }
