...
class member_complainControl extends BaseMemberControl{
     ...
     * 新投诉
     */
    public function complain_newFeiwa() {
        $order_id = intval($_GET['order_id']);
        $goods_id = intval($_GET['goods_id']);//订单商品表编号
        if ($order_id < 1 || $goods_id < 1) {//参数验证
            showMessage(Language::get('wrong_argument'),'index.php?app=member_order&feiwa=index','html','error');
        }
        $condition = array();
     ...
     * 处理投诉请求
     */
    public function complain_showFeiwa() {
     ...
     * 保存用户提交的投诉
     */
    public function complain_saveFeiwa() {
        //获取输入的投诉信息
        $input = array();
        ...
        if($if_complain < 1) {
            showDialog(Language::get('para_error'),'index.php?app=member_order&feiwa=index','error');
        }
        //检查是不是正在进行投诉
        ...
        if ($state) {
            showDialog(Language::get('complain_submit_success'),'index.php?app=member_complain&feiwa=index','succ');
        } else {
     ...
     * 保存用户提交的补充证据
     */
    public function complain_add_picFeiwa() {
        $complain_id = intval($_GET['complain_id']);
        //获取投诉详细信息
        $complain_info = $this->get_complain_info($complain_id);
        if (chksubmit()){
     ...
     * 取消用户提交的投诉
     */
    public function complain_cancelFeiwa() {
        $complain_id = intval($_GET['complain_id']);
        $complain_info = $this->get_complain_info($complain_id);
     ...
     * 处理用户申请仲裁
     */
    public function apply_handleFeiwa() {
        ...
        showMessage(Language::get('handle_submit_success'),'index.php?app=member_complain');
    }
    ...
    private function profile_menu($menu_key='') {
        $menu_array = array(
            array('menu_key'=>'complain_accuser_list','menu_name'=>Language::get('complain_manage_title'),'menu_url'=>'index.php?app=member_complain')
        );
