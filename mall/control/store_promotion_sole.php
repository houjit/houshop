...
class store_promotion_soleControl extends BaseSellerControl {
    ...
    public function indexFeiwa() {
        $this->sole_goods_listFeiwa();
    }

    /**
     * 套餐商品列表
     */
    public function sole_goods_listFeiwa() {
     ...
     * 选择商品
     */
    public function sole_select_goodsFeiwa() {
     ...
     * 购买套餐
     */
    public function sole_quota_addFeiwa() {
        if (chksubmit()) {
            ...
            if ($quantity <= 0 || $quantity > 12) {
                showDialog('参数错误，购买失败。', urlMall('store_promotion_sole', 'sole_quota_add'), '', 'error' );
            }
            // 实例化模型
            $model_sole = Model('p_sole');
            ...
            if ($return) {
                ...
                showDialog('购买成功', urlMall('store_promotion_sole', 'sole_goods_list'), 'succ');
            } else {
                showDialog('购买失败', urlMall('store_promotion_sole', 'sole_quota_add'));
            }
        }
        // 输出导航
     ...
     * 套餐续费
     */
    public function sole_renewFeiwa() {
        if (chksubmit()) {
            $model_sole = Model('p_sole');
            $quantity = intval($_POST['sole_quota_quantity']); // 购买数量（月）
            $price_quantity = $quantity * intval(C('promotion_sole_price')); // 扣款数
            ...
            if ($return) {
                ...
                showDialog('购买成功', urlMall('store_promotion_sole', 'sole_list'), 'succ');
            } else {
                showDialog('购买失败', urlMall('store_promotion_sole', 'sole_quota_add'));
            }
        }
     ...
     * 保存选择商品
     */
    public function choosed_goods_saveFeiwa() {
        $model_sole = Model('p_sole');
        if (!checkPlatformStore()) {
            // 验证套餐时候过期
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            case 'sole_goods_list':
                $menu_array = array(
                    1=>array('menu_key'=>'sole_goods_list', 'menu_name'=>'商品列表', 'menu_url'=>urlMall('store_promotion_sole', 'sole_goods_list'))
                );
                break;
            case 'sole_quota_add':
                $menu_array = array(
                    1=>array('menu_key'=>'sole_goods_list', 'menu_name'=>'商品列表', 'menu_url'=>urlMall('store_promotion_sole', 'sole_goods_list')),
                    2=>array('menu_key'=>'sole_quota_add', 'menu_name'=>'购买套餐', 'menu_url'=>urlMall('store_promotion_sole', 'sole_quota_add'))
                );
                break;
            case 'sole_renew':
                $menu_array = array(
                    1=>array('menu_key'=>'sole_goods_list', 'menu_name'=>'商品列表', 'menu_url'=>urlMall('store_promotion_sole', 'sole_goods_list')),
                    2=>array('menu_key'=>'sole_renew', 'menu_name'=>'套餐续费', 'menu_url'=>urlMall('store_promotion_sole', 'sole_renew'))
                );
                break;
        }
