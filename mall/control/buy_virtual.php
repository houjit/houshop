...
class buy_virtualControl extends BaseBuyControl {
     ...
     * 虚拟商品购买第三步
     */
    public function buy_step3Feiwa() {
        $logic_buy_virtual = Logic('buy_virtual');
        $_POST['order_from'] = 1;
        ...
        //转向到商城支付页面
        redirect('index.php?app=buy_virtual&feiwa=pay&order_id='.$result['data']['order_id']);
    }
    ...
    public function payFeiwa() {
        ...
        if ($order_id <= 0){
            showMessage('该订单不存在','index.php?app=member_vr_order','html','error');
        }
        ...
        if (empty($payment_list)) {
            showMessage('暂未找到合适的支付方式','index.php?app=member_vr_order','html','error');
        }
