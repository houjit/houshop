...
class store_consultControl extends BaseSellerControl {
    ...
    public function indexFeiwa(){
        $this->consult_listFeiwa();
    }

    /**
     * 商品咨询列表页
     */
    public function consult_listFeiwa(){
     ...
     * 商品咨询删除处理
     */
    public function drop_consultFeiwa(){
        $ids = trim($_GET['id']);
     ...
     * 回复商品咨询表单页
     */
    public function reply_consultFeiwa(){
        $consult = Model('consult');
        $list_consult = array();
        $search_array = array();
     ...
     * 商品咨询回复内容的保存处理
     */
    public function reply_saveFeiwa(){
        ...
        if($state){
            ...
            $param['param'] = array(
                ...
                'consult_url' => urlMall('member_consult', 'my_consult')
            );
            QueueClient::push('sendMemberMsg', $param);

            showDialog(Language::get('feiwa_common_op_succ'),'reload','succ',empty($_GET['inajax']) ?'':'CUR_DIALOG.close();');
        } else {
    ...
    private function profile_menu($menu_type,$menu_key='',$array=array()) {
        ...
        switch ($menu_type) {
            case 'consult':
                $menu_array = array(
                1=>array('menu_key'=>'consult_list',    'menu_name'=>Language::get('feiwa_member_path_all_consult'),           'menu_url'=>'index.php?app=store_consult&feiwa=consult_list'),
                2=>array('menu_key'=>'to_reply',    'menu_name'=>Language::get('feiwa_member_path_unreplied_consult'),         'menu_url'=>'index.php?app=store_consult&feiwa=consult_list&type=to_reply'),
                3=>array('menu_key'=>'replied', 'menu_name'=>Language::get('feiwa_member_path_replied_consult'),           'menu_url'=>'index.php?app=store_consult&feiwa=consult_list&type=replied'));
                break;
        }
        if(!empty($array)) {
            $menu_array[] = $array;
