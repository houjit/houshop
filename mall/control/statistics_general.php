...
class statistics_generalControl extends BaseSellerControl {
     ...
     * 价格区间设置
     */
    public function pricesettingFeiwa(){
     ...
     * 订单价格区间设置
     */
    public function orderprangeFeiwa(){
        $model = Model('store_extend');
        if (chksubmit()){
            $update_array = array();
            if ($_POST['pricerange']){
    ...
    /*private function profile_menu($menu_key='') {
        $menu_array = array(
            1=>array('menu_key'=>'stat_general','menu_name'=>'店铺概况',    'menu_url'=>'index.php?app=statistics_general&feiwa=general')
        );
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            case 'setting':
                $menu_array = array(
                    1=>array('menu_key'=>'pricesetting','menu_name'=>'商品价格区间',    'menu_url'=>'index.php?app=statistics_general&feiwa=pricesetting'),
                    2=>array('menu_key'=>'orderprange','menu_name'=>'订单金额区间', 'menu_url'=>'index.php?app=statistics_general&feiwa=orderprange'));
                break;
        }
        Tpl::output('member_menu',$menu_array);
        Tpl::output('menu_key',$menu_key);
